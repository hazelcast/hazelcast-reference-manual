...
- `initial-mode`: Sets the initial load mode. LAZY is the default load mode, where load is asynchronous. EAGER means load is blocked till all partitions are loaded. Please see the [Initializing Map on Startup section](/600_Loading_and_Storing_Persistent_Data.md#page_Initializing+Map+on+Startup) for more details.

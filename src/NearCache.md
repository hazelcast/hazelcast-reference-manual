...
To solve this problem, starting from version 3.8, Hazelcast provides eventually consistent behaviour for IMap/JCache Near Caches by detecting invalidation losses.
After detection of invalidation loss, stale data will be made unreachable and Near Cache get calls to that data will be directed to underlying IMap/JCache to fetch fresh data.

You can configure eventual consistency with the system properties below (same properties are valid both member and client side Near Caches):

...
* `migrationStarted`: Migration starts. Migration consists of a group of replica migrations
which are planned together. Parameter of `migrationStarted` method, `MigrationState`,
shows information about the migration; start time of the process, number of the planned migrations etc.
* `migrationFinished`: Migration finishes. `MigrationState` parameter shows the result of the migration;
number of the completed migrations, number of the remaining migrations, total elapsed time etc.
* `replicaMigrationCompleted`: A partition replica migration is started. Method's parameter,
`ReplicaMigrationEvent`, shows information about a replica migration; partition ID, replica index, source
and destination members of the migration and elapsed time for this replica migration.
...
* `replicaMigrationFailed`: A partition replica migration failed. Parameter `MigrationEvent`
shows the information about this replica migration
and the overall migration similar to `migrationCompleted` method.

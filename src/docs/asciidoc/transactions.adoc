...
`MapStore` and `QueueStore` do not participate in transactions. Hazelcast suppresses exceptions thrown by the store in a transaction. Please refer to the <<providing-xa-transactions, XA Transactions section>> for further information.

...
`MapStore` and `QueueStore` do not participate in transactions. Hazelcast suppresses exceptions thrown by the store in a transaction. See the <<providing-xa-transactions, XA Transactions section>> for further information.

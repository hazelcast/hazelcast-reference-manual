...
Please refer to C++ client's own GitHub repo at https://github.com/hazelcast/hazelcast-cpp-client[Hazelcast C++ Client] for information on setting the client up, installing and compiling it, its serialization support, and APIs such as raw pointer and query. 

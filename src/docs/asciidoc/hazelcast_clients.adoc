...
When client trying to connect to cluster, it uses following methods
Please see <<setting-connection-attempt-limit, Setting Connection Attempt Limit>>.
Please see <<setting-connection-attempt-period, Setting Connection Attempt Period>>.

When failed to connect to any member in memberlist, it waits for `connection-attempt-period` then tries this
`connection-attempt-limit` and fails to connect. 

With Connection Retry Config, you can fine tune this connection behaviour. When Connection Retry Config is enabled, 
`connection-attempt-period` and `connection-attempt-limit` are not used anymore. 
This configuration is used when trying to establish the initial connection and  any time when connection to cluster is broken.

initial-backoff-millis (int) :  how long to wait after the first failure before retrying
max-backoff-millis (int): upper bound on backoff
multiplier (double): factor with which to multiply backoff after a failed retry
fail-on-max-backoff (boolean): whether to fail when the max-backoff has reached or continue waiting max-backoff-millis at each iteration
jitter (double): by how much to randomize backoffs
...
```
Note that, TryConnect tries to connect any member that client knows of, and for each connection we have a connection timeout. 
see <<setting-connection-timeout, Setting Connection Timeout>>.

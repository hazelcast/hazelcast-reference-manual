...
Hundreds or even thousands of clients can be connected to the cluster. By default, there are `core count * 20` threads on the server side that handle all the requests, e.g., if the server has 4 cores, there will be 40 threads.
...
The first step is configuration. You can configure the Java client declaratively or programmatically. We use the programmatic approach for this section, as shown below.
...
As a final step, if you are done with your client, you can shut it down as shown below. This releases all the used resources and closes connections to the cluster.
...
You can refer to the <<aws-element, aws element section>> for the descriptions of the above AWS configuration elements except `inside-aws` and `iam-role`, which are explained below.
...
Hazelcast provides disaster recovery for the client-cluster connections and can use the well-known blue-green concept, so that a Java client is automatically diverted to another cluster on demand or when the intended cluster becomes unavailable.
...
For the blue-green scenario

The user will blacklist the client via the Management Center. Blacklisting is defined under different section here.
When a client is disconnected, it will first try another member in the current cluster. This is because the client is not aware if this is a blacklisting or normal disconnect.
Behavior, when disconnected, will change depending on RECONNECT-MODE as before.

When alternative clusters are listed,
1. RECONNECT-MODE ON. changes cluster and block invocations while doing so.
2. RECONNECT-MODE ASYNC
changes cluster in the background and Throw ClientOfflineException to the user while doing so.
3. RECONNECT-MODE OFF
Does not change cluster. shuts down directly.

Here it could be the case that whole cluster restarted. In that case, the restarted server will reject the client since the client is trying to an old cluster. Then, we will continue from item 6. To achieve this, we would be sending cluster uuid on authentication request when multiple configs are provided. Client will return NOT_ALLOWED_IN_CLUSTER status as a response if it is restarted.
When a client tries to authenticate to a cluster where it is blacklisted, it will get NOT_ALLOWED_IN_CLUSTER as the response of Authentication. See this for details.
New NOT_ALLOWED_IN_CLUSTER means the client needs to search for a new cluster, if available. For the connect order, see this 
When a failover started, we will not use current member-list disconnect all connections before attempting to connect to a new cluster. See this section for cleanups we do when switching clusters
We will not fire Member ADDED or REMOVED events when disconnected from the old cluster. Instead, we will fire InitialMembershipListener#init(InitialMembershipEvent event); when we switch clusters.
Also a new lifecycle event CLIENT_CHANGED_CLUSTER will be fired.
When switching clusters client will reuse the client uuid. 
ListenerService will re-register listeners to the new cluster. Technically this should not require new code. Listener service opens new connection to all members in current member-list and register listeners to each connection.
Near caches and continuous query caches will be cleared if joining new cluster is successful.
If in the new cluster partition size is different, the client will be rejected by the cluster. see this for details

===== Disaster Recovery Mechanism

The cluster is gone. Owner connection is disconnected. 
When a client is disconnected, it will first try another member in the current cluster disconnect.
Behavior, when disconnected, will change depending on RECONNECT-MODE as before.
...
Client will exhaust all related configuration (connection-attempt limit or exponential backoff strategy). 
If alternative clusters are given, the client will try to connect to alternative clusters. For the connect order, see this section
...
When switching clusters client will reuse the client uuid. 
ListenerService will re-register listeners to the new cluster. Technically this should not require new code. Listener service opens new connection to all members in current member-list and register listeners to each connection.
Near caches and continuous query caches will be cleared if joining a new cluster is successful.
If in the new cluster partition size is different, the client will be rejected by the cluster. see this for details
...
Let's describe the configuration with examples. Assume that you have two clusters, Cluster A and Cluster B, and two Java clients.
...
|block
|string
|Specifies whether the response thread for internal operations at the client side is blocked or not. If you use `block` (the default value), the thread is blocked and needs to be notified which can cause a reduction in the performance. If you use `backoff` there is no blocking. By enabling the backoff mode and depending on your use case, you can get a 5-10% performance improvement. However, keep in mind that this increases the CPU utilization. We recommend you to use backoff with care and if you have a tool for measuring your cluster's performance.
|===

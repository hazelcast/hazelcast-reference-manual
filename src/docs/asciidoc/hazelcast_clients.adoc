...
You can define attributes in your Java client, similar to the way it can be done for the <<defining-member-attributes, members>>. Through the client attributes, you can assign special roles for your clients and use these roles to perform some actions specific to those client connections.

You can also group your clients using the client attributes. These client groups can be blacklisted in the Hazelcast Management Center so that they can be prevented from connecting to a cluster. See the related section in the Hazelcast Management Center Reference Manual for more information on this topic.

Declaratively, you can define the client attributes using the `client-attributes` configuration element. See the below example.
...
<hazelcast-client>
    ...
    <client-attributes>
        ...
        <attribute name="region">bar</attribute>
    </client-attributes>
    ....
</hazelcast-client>
----
...
clientConfig.setAttribute("role", "user");
clientConfig.setAttribute("region", "bar");

HazelcastClient.newHazelcastClient(clientConfig);
----

See the https://github.com/hazelcast/hazelcast-code-samples/tree/master/clients/client-attributes[code sample] for the client attributes to see them in action.

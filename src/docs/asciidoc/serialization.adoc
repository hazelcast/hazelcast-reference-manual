...
`IdentifiedDataSerializable` uses `getId()` instead of class name and it uses
...
The methods `getId` and `getFactoryId` return a unique positive number within
...
link:https://docs.hazelcast.org/docs/latest/javadoc/com/hazelcast/nio/serialization/VersionedPortable.html[VersionedPortable^]

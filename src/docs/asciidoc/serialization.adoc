...
Hazelcast offers a Java deserialization protection based on blacklisting and whitelisting the class/package names.

This protection is controlled using the following system properties: 

* `hazelcast.serialization.filter.enabled`: Enables Java deserialization protection which is a filtering mechanism based on blacklists and whitelists. Its default value is false.
* `hazelcast.serialization.filter.blacklist.classes`: Comma separated list of blacklisted class names. Its default value is empty.
* `hazelcast.serialization.filter.blacklist.packages`: Comma separated list of blacklisted package names. Its default value is empty.
* `hazelcast.serialization.filter.whitelist.classes`: Comma separated list of whitelisted class names. Its default value is empty.
* `hazelcast.serialization.filter.whitelist.packages`: Comma separated list of whitelisted package names. Its default value is empty.

NOTE: Please see <<configuring-with-system-properties, Configuring with System Properties>> for information on how to set system properties.
...
When deserialization fails, a `SecurityException` is thrown. If both `hazelcast.serialization.filter.blacklist.classes` and `hazelcast.serialization.filter.blacklist.packages` properties are empty and deserialization protection is enabled, then a hardcoded list of well known vulnerable classes and packages is used.

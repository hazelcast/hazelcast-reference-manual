...
Firstly, `writePortable` method cannot be called with a `null` object. `writeNullPortable` should be used instead as shown below:
...
Secondly, when the user tries to serialize a null portable when there is no class definition at the moment, Hazelcast throws a serialization exception saying "Cannot write null portable

...
javaSerializationFilterConfig.getBlacklist().addClasses(SomeDeserialized.class.getName());
...
Once the feature is enabled, the following filtering rules are used when objects are deserialized.
...
When deserialization fails, a `SecurityException` is thrown. When the blacklist is not explicitly provided, a default hardcoded value with some well known vulnerable class names is used.

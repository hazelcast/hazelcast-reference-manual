...
NOTE: The term "split brain protection" used in this section simply refers to
...
This is achieved by defining and configuring a Split-Brain Protection for the cluster.
If the cluster size is below the defined split brain protection size, the operations are rejected and
the rejected operations return a `SplitBrainProtectionException` to their callers.
Additionally, it is possible to configure a split brain protection with a user-defined
...
configured with new out-of-the-box ``SplitBrainProtectionFunction``s which
...
You can set up Split-Brain Protection configuration using either
...
The probabilistic split brain protection function has the following configuration elements:
...
A split brain protection with a recently-active split brain protection function can be used to implement
...
* `probabilistic-split-brain-protection`: Configures the split brain protection with a probabilistic split
brain protection function. It cannot be used in conjunction with `split-brain-protection-function-class-name` or
...
split brain protection function. It cannot be used in conjunction with `split-brain-protection-function-class-name`
...
You can register split brain protection listeners to be notified about split brain protection results.
...
Split brain protection listeners can be configured via declarative or programmatic configuration.
...
SplitBrainProtection instances let you query the result of a particular split brain protection.

Here is a SplitBrainProtection interface that you can interact with.
...
You can retrieve the SplitBrainProtection instance for a particular split brain protection over
the split brain Protection service, as in the following example.

...
determine there is no split-brain independently of the cluster membership manager,
...
In some cases, detecting and handling network partitions is not very straightforward.
...
Assume `[N1, N2, N3, N4]` is a cluster with 4 members. This is how a healthy cluster looks,
each member has a connection to other members:

image::HealthyCluster.png[Healthy Cluster]

When `N2` becomes partitioned away from rest of the cluster when it disconnects from
other members, and eventually 2 separate groups are formed: `[N1, N3, N4]` and `[N2]`.
This is called a full network partition:

image::FullNetworkPartition.png[Full Network Partition]
...
`N3` and `N4`, two overlapping healthy groups will be formed: `[N1, N2]` and `[N1, N3, N4]`.

image::PartialNetworkPartition1.png[Partial Network Partition]

Or when `N2` is disconnected only from `N1`, again two overlapping healthy groups will be
...
this problem is to figure out the largest set of fully-connected members and artifically separate
these members from the rest. This way we will have completely separated set of members without
any intersection. This is the same problem with the link:https://en.wikipedia.org/wiki/Clique_problem[maximum clique problem^]
in graph theory. Hence we are using an implementation of the link:https://en.wikipedia.org/wiki/Bron%E2%80%93Kerbosch_algorithm[Bron–Kerbosch algorithm^]
...
we need to gather those local failure detection data from all members. When partital network
partitioning resolution mechanism is enabled, all members send their local failure detections (suspicions about other members)
inside the usual heartbeat-message. (See the <<failure-detector-configuration, Failure Detector Configuration section>>
for more info about failure detectors.) The master (oldest) member in the cluster gathers all this
information and executes the max clique algorithm and then decides the smallest set of Hazelcast members,
if any, to kick from the cluster so that the remaining members are fully-connected to each other again.
In a healtly cluster this set will be empty.
...
disconnection resolution mechanism uses a graph algorithm that finds a max-clique in non-polynomial time.
Since it could take a lot of time to find a max-clique in a large graph, i.e, in a large cluster with

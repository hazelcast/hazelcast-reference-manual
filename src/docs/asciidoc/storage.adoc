...
makes sure every allocation is done in a different directory then the last.
This is a best-effort attempt to distribute the allocations evenly on the
persistent memory DIMMs, which is important from utilization and performance
point of view as well. This is the default allocation strategy.
...
strategy for the operation threads. An example configuration for that:
`-Dhazelcast.operation.thread.affinity=[0-9,20-29]:20,[10-19,30-39]:20`. This
configuration restricts all 40 operation threads to run on a single NUMA node
...
for every persistent memory directory in the configuration. If both configuration
is done properly, the threads in the thread groups restricted to run on a single
...
Please check which mount point represents which PMEM device in the output of
`ndctl`.
...
of Hazelcast if its stores its data on persistent memory or on regular memory.
Such a use case is caching, where accessing the entries remotely through
Hazelcast clients results in very similar throughput. Based on our tests with

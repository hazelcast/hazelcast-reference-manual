...
+
NOTE: Previously, `base-dir` was being used only by a single Hazelcast member. If such an existing `base-dir` is configured for a Hazelcast member, Hot Restart will start in legacy mode and `base-dir` will be used only by a single member, without creating a unique sub-directory. Other members trying to use that `base-dir` fails during the startup.

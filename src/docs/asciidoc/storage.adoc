...
* **size:** Size of the total native memory to allocate in megabytes. Default value is **512 MB**.
...
* **minimum block size:** Minimum size of the blocks in bytes to split and fragment a page block to assign to an allocation request. It is used only by the **POOLED** memory allocator. Default value is **16 bytes**.

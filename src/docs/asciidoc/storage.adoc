...
NOTE: You can check whether there is enough free physical memory for the requested number of bytes using the system property `hazelcast.hidensity.check.freememory`. Please see the <<system-properties, System Properties appendix>> on how to use Hazelcast system properties.
...
* `backup-dir`: Specifies the directory under which Hot Restart snapshots (Hot Backups) are stored. Please see the <<hot-backup, Hot Backup section>> for more information.

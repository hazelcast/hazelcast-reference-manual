...
==== Using Task Scheduling with IScheduledExecutorService

As mentioned in the <<scheduled-executor-service, Scheduled Executor Service section>>, you can use Hazelcast's IScheduledExecutorService within Spring context for your task scheduling needs. For this, you simply configure your Hazelcast instance, specify a name for the scheduler and create it in Hazelcast. Following is an example configuration depicting this:
...
<hz:hazelcast id="instance">
    <hz:config>
        [...]
        <hz:scheduled-executor-service name="my-scheduler" pool-size="10" />
    </hz:config>
</hz:hazelcast>

<task:annotation-driven scheduler="my-task-scheduler" />
<hz:scheduledExecutorService id="my-task-scheduler" instance-ref="instance" name="my-scheduler" />
...
----

Here, the tag `<hz:hazelcast>` configures your Hazelcast member with the ID "instance". In the Spring tag `<task:annotation-driven>`, you specify a name for the Java annotated scheduler. Finally, the tag `<hz:scheduledExecutorService>` creates a scheduler for your Hazelcast "instance" with the name "my-scheduler".

After you create and configure a scheduler as exemplified above, you can run the `scheduledMethod()` with that newly created scheduler as shown below:
...
You can refer to Spring's https://docs.spring.io/spring/docs/current/spring-framework-reference/integration.html#scheduling-task-scheduler[documentation] to learn about it's task scheduling feature.

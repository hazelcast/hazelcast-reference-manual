...
<hz:scheduledExecutorService id="scheduler" instance-ref="instance" name="test-scheduler" />
...
----

Here, the tag `<hz:hazelcast>` configures your Hazelcast member with the ID "instance". In the Spring tag `<task:annotation-driven>`, you specify a name for the Java annotated scheduler. Finally, the tag `<hz:scheduledExecutorService>` creates a scheduler for your Hazelcast "instance" with the name "test-scheduler".

You can refer to Spring's https://docs.spring.io/spring/docs/3.2.x/spring-framework-reference/html/scheduling.html[documentation] to learn about it's task scheduling feature.

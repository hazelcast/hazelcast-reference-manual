...
See <<cp-membership-listener, CP Membership Listener section>> to get notified
about CP member additions and removals.
...
`CPMembershipListener` can be defined in configuration or can be registered
in runtime via `CPSubsystem` API.

Below is an example registering the listener in runtime using method `CPSubsystem.addMembershipListener`.
...
`CPGroupAvailabilityListener` is notified when availability of a CP group decreases
or it loses the majority completely.

In general, availability decreases when a CP member becomes unreachable because of process crash,
...
loss of majority. When majority of a CP group is lost, that CP group cannot make
...
in configuration or can be registered in runtime via `CPSubsystem` API.

Below is an example registering the listener in runtime using method `CPSubsystem.addGroupAvailabilityListener`.

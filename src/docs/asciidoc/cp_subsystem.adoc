...
WARNING: CP Subsystem operates in **unsafe mode** by default,
without proving strong consistency guarantee. See <<cp-subsystem-unsafe-mode, CP Subsystem Unsafe Mode>>
for more information about _unsafe_ mode.
To enable CP Subsystem, set CP member count to a value greater than `0`.
See <<cp-subsystem-configuration, CP Subsystem Configuration>> section for configuration details.
...
When CP Subsystem is not enabled, that means when `CPSubsystemConfig.getCPMemberCount()` is `0`, CP data structures will operate in _unsafe_ mode. In _unsafe_ mode, they will use Hazelcast's partitioning system and lazy replication instead of CP Subsystem's consensus mechanism. For more information about lazy replication, see the <<consistency-and-replication-model, Consistency and Replication Model chapter>>.

_Unsafe_ mode provides weaker consistency guarantees. For example, when you increment an `IAtomicLong` or acquire a `FencedLock`, after crash of a member, even though caller of the API receives a success response, client's write operation (increment of `IAtomicLong` or acquire of `FencedLock`) can be lost. That's why, it's not adviced to be used in systems requiring strong consistency. _Unsafe_ mode is more suitable to be used during development or while testing.

NOTE: In _unsafe_ mode, CP Subsystem Management API will not be available.
...
[source,sh]
----
> curl -X POST --data "${CLUSTERNAME}&${PASSWORD}" http://127.0.0.1:5701/hazelcast/rest/cp-subsystem/groups/${CPGROUP_NAME}/remove
OR
> sh cp-subsystem.sh -o force-destroy-group --group ${CPGROUP_NAME} --address 127.0.0.1 --port 5701 --clustername ${CLUSTERNAME} --password ${PASSWORD}
----
+
* **Remove a CP Member:**
+
...
[source,sh]
----
> curl -X POST --data "${CLUSTERNAME}&${PASSWORD}" http://127.0.0.1:5701/hazelcast/rest/cp-subsystem/members/${CPMEMBER_UUID}/remove
OR
> sh cp-subsystem.sh -o remove-member --member ${CPMEMBER_UUID} --address 127.0.0.1 --port 5701 --clustername ${CLUSTERNAME} --password ${PASSWORD}
----
+
...
[source,sh]
----
> curl -X POST --data "${CLUSTERNAME}&${PASSWORD}" http://127.0.0.1:5701/hazelcast/rest/cp-subsystem/members
OR
> sh cp-subsystem.sh -o promote-member --address 127.0.0.1 --port 5701 --clustername ${CLUSTERNAME} --password ${PASSWORD}
----
+
...
[source,sh]
----
> curl -X POST --data "${CLUSTERNAME}&${PASSWORD}" http://127.0.0.1:5701/hazelcast/rest/cp-subsystem/restart
OR
> sh cp-subsystem.sh -o restart --address 127.0.0.1 --port 5701 --clustername ${CLUSTERNAME} --password ${PASSWORD}
----
...
[source,sh]
----
> curl -X POST --data "${CLUSTERNAME}&${PASSWORD}" http://127.0.0.1:5701/hazelcast/rest/cp-subsystem/groups/${CPGROUP_NAME}/sessions/${CP_SESSION_ID}/remove
OR
> sh cp-subsystem.sh -o force-close-session --group ${CPGROUP_NAME} --session-id ${CP_SESSION_ID} --address 127.0.0.1 --port 5701 --clustername ${CLUSTERNAME} --password ${PASSWORD}
----

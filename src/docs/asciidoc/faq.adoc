...
(see link:https://en.wikipedia.org/wiki/CAP_theorem[CAP theorem^]).
...
same state with the primary. Backup replicas can be used to scale reads (see the <<enabling-backup-reads, Enabling Backup Reads section>>)
...
** See the <<consistency-and-replication-model, Consistency and Replication Model chapter>>
...
** It provides a good degree of fault tolerance at run-time, and <<cp-subsystem-persistence, CP Subsystem Persistence>> enables more robustness.
** See the <<cp-subsystem, CP Subsystem chapter>> for more information.

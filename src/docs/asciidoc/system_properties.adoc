...
|[[permission-onjoin]]`hazelcast.security.permissions.operation.on-join`
|RECEIVE
|bool
|It is used to specify whether a new member joining to a cluster will apply the <<permissions, client permissions>> stored in its own configuration and send them to the cluster, or use the ones configured in the cluster. Its default value is RECEIVE, i.e., the joining member uses the client permissions defined in the cluster. Available other values are `SEND` and `NONE`. See the <<handling-permissions-when-a-new-member-joins, Handling Permissions When a New Member Joins section>>.

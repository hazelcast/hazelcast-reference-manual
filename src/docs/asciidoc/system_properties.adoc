...
pre-join phase ended, the master moves into the join phase, during which it
...
make their first join request. Once `hazelcast.wait.seconds.before.join`
elapsed since the last first-timer join request (i.e. where the member hasn't

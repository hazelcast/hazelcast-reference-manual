...
Split-Brain protection for Executor Service can be configured programmatically using the method link:https://docs.hazelcast.org/docs/3.10/javadoc/com/hazelcast/config/ExecutorConfig.html[setQuorumName()^], or declaratively using the element `quorum-ref`. Following is an example declarative configuration:
...
* `quorum-ref`: Name of quorum configuration that you want this Scheduled Executor Service to use. See the <<split-brain-protection-for-ischeduled-executor-service, Split-Brain Protection for IScheduled Executor Service section>>.

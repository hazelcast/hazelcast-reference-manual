...
* `statistics-enabled`: Specifies whether the statistics gathering will be enabled for your Executor Service. If set to `false`, you cannot collect statistics in your implementation (using `getLocalExecutorStats()`) and also https://docs.hazelcast.org/docs/management-center/latest/manual/html/index.html#monitoring-executors[Hazelcast Management Center] will not show them. Its default value is `true`.

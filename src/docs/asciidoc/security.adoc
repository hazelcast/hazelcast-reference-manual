...
NOTE: You can choose whether a new member joining to a cluster will apply the client permissions stored in its own configuration, or will use the ones defined in the cluster. The decision for this can be set using the <<permission-onjoin, `hazelcast.security.permission.update.onjoin.disable`>> system property. Its default value is `false`, meaning that the new member applies the permissions defined in the cluster. If you set this property to `true` for a member and it has different permission configurations than the cluster, the new member and other cluster members will behave differently in terms of using client permissions.

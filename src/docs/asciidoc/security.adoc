...
[source,xml,indent=0,subs="verbatim,attributes",role="primary"]
.Sample Kerberos Authentication Configuration XML
----
<realm name="kerberosRealm">
    <authentication>
        <kerberos>
...
Hazelcast Enterprise allows observing some important cluster events by the Auditlog feature.
Auditable events have a unique type ID, they contain a timestamp and importance level.
Events may also contain a message and parameters.
Supported event type identifiers are listed in link:{docBaseUrl}/javadoc/com/hazelcast/auditlog/AuditlogTypeIds.html[`AuditlogTypeIds`^].

Auditlog can be enabled in the Hazelcast configuration:
...
as log entries with category name `"hazelcast.auditlog"`.
...
The Auditlog has own SPI allowing users to provide own implementations. Relevant classes and interfaces are located
in the link:{docBaseUrl}/javadoc/com/hazelcast/auditlog/package-summary.html[`com.hazelcast.auditlog` package^].

The central point of auditlog SPI is the
link:{docBaseUrl}/javadoc/com/hazelcast/auditlog/AuditlogService.html[`AuditlogService` interface^]
and its `log(...)` methods. Their implementation is responsible for processing auditable events (e.g. writing them to a database).

The `AuditlogService` also creates the
link:{docBaseUrl}/javadoc/com/hazelcast/auditlog/EventBuilder.html[`EventBuilder`^]
instances which are used to build
link:{docBaseUrl}/javadoc/com/hazelcast/auditlog/AuditableEvent.html[`AuditableEvents`^].

Another important piece in the SPI is the
link:{docBaseUrl}/javadoc/com/hazelcast/auditlog/AuditlogServiceFactory.html[`AuditlogServiceFactory` interface^].

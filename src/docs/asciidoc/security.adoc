...
with the `keytabFile` to replace `security-realm` configuration.

_We don't recommend using this property in production!_

| `keytabFile`
|
| Path to a keytab file with current principal's  secrets.
This is a helper option which can be used together
with the `principal` to replace `security-realm` configuration.
...
Instead it's possible to define the `principal` and `keytabFile` options in
`kerberos` identity and authentication configuration.
...
It includes the generated configuration, so users can use it as a starting point
for defining full Kerberos configuration. Example:

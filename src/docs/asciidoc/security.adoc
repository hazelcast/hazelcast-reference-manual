...
link:https://github.com/openssl/openssl/blob/master/INSTALL.md[github.com/openssl^].
...
* `kerberos`: verifies Kerberos token provided in `TokenCredentials`
...
| `simple`
| Name of the authentication mechanism used for admin LDAP connection.
It's used as a value
for JNDI environment property `Context#SECURITY_AUTHENTICATION`.
Users can specify `GSSAPI` to authenticate with Kerberos protocol.
...
| `security-realm`
|
a| If specified then given realm name is used for authentication of
...
for single-sign-on. It's well established in many companies. Hazelcast
...
and authentication. The identity part is responsible for retrieving service
ticket from Kerberos KDC (Key Distribution Center).
The authentication part verifies service tickets.
...
authenticated - the TGT (Ticket granting ticket) is present in the Subject.

Hazelcast delegates the TGT retrieval to vendor specific `Krb5LoginModule`
implementations. Find the correct login module and its opstions in your Java
documentation. On Hazelcast side, the `security-ream` property allows
referencing another realm with `Krb5LoginModule` configured.

[source,xml]
.Sample Kerberos Identity Configuration
----
...
meant for use-cases where all members share a single Kerberos identity.

| `service-name-prefix`
| `"hz/"`
| Defines prefix of the Service Principal
Name. By default the member's principal name (for which this credentials
factory asks the service ticket) is in form
...
| `security-realm`
|
| Security realm name in Hazelcast configuration used
for Kerberos authentication. The authentication configuration in the
referenced security realm will be used to fill the Subject with Kerberos
credentials (e.g. TGT).
|=======================================================================

The authenticating part on the server-side is able to
accept the Kerberos tickets and verify them. Again the Kerberos
authentication is delegated to another realm with Kerberos login module
configured.

[source,xml]
.Sample Kerberos Authentication Configuration
----
<realm name="kerberosRealm">
    <authentication>
        <kerberos>
...
The `krb5Acceptor` realm configuration in the snippet only loads Kerberos secrets from
...
* The GSS-API is not used for protecting (wrapping) messages after
authentication (e.g. encryption, integrity checks). It's only used for
accepting tokens.
* The token itself is not protected against Man-in-the-Middle attacks.
...
** There is a replay protection in Java which caches already used tokens.
** Java Kerberos implementation accepts the token for 5min (by default)
from its creation.
...
member's Kerberos credentials to authenticate into the LDAP.
...
Hazelcast 4.1 introduces a limited support of security realms in native clients.

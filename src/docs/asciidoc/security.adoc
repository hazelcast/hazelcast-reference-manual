...
Clients don't need to set `mutualAuthentication` property as it's property configuring server side of TLS connections.

==== Mutual Authentication

TLS connections have two sides - one which opens the connection (i.e. TLS client)
and other one which accepts the connection (TLS server).
By default only the TLS server proves its identity by presenting a certificate to the TLS client.
The Mutual authentication means that also TLS clients prove their identity to TLS servers.

Hazelcast members can be on both sides of TLS connection - TLS servers and TLS clients.
Hazelcast clients are always on client side of a TLS connection.

By default Hazelcast members have keyStore used to identify themselves to clients and other members.
Both Hazelcast members and Hazelcast clients have trustStore used to define which members they can trust.

When the Mutual authentication feature is enabled, Hazelcast clients need to have keyStore provided.
A client proves its idenity by providing its certificate to Hazelcast member it's connecting to.
...
Therefor it's recommended to require the mutual authentication in Hazelcast members configuration.

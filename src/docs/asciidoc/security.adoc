...
Hazelcast 4 introduces a new security configuration - security realms.
...
The realm can't have more than one authentication method specified.

Available authentication types:

* `jaas` - allows defining JAAS login module stacks
* `ldap` - verifies `PasswordCredentials` against an LDAP server
* `tls` - verifies that the TLS mutual authentication was
  used in the incoming connection and the peer's certificate chain is
  available.
...
Sample configuration, which authenticates against an LDAP server or Database as a fallback:
...
For more details look into <<jaas-authentication, JAAS authentication>> section.

===== LDAP authentication type

LDAP servers are one of the most popular identity stores in companies. They can track information about organization structure, users, groups, servers and configurations.

Hazelcast supports authentication and authorization against LDAP servers. The authentication verifies provided name and password. The authorization part allows to map roles to the authenticated user.

The password verification during authentication is possible by 2 means:

* making a new LDAP bind operation with given name and password
* using a separate "admin connection" to verify provided password against an LDAP object attribute
...
* `direct` mapping: the identity object contains an attribute with reference to role object(s);
* `reverse` mapping: role objects having a reference to the identity object are searched.

The `direct` and `reverse` mapping modes will also allow a role search recursion.
...
| url
|
| URL of the LDAP server. The value is configured as JNDI environment property `java.naming.provider.url`.

| socket-factory-class-name
|
| Socket factory class name. The factory can be used for fine grained configuration of TLS protocol on top of LDAP protocol (i.e. `ldaps` scheme)

| parse-dn
| false
| If set to `true`, then it treats the value of `role-mapping-attribute` as a DN and extracts only `role-name-attribute` attribute values as role names. When the `parse-dn` option value is `false`, then the whole value of `role-mapping-attribute` is used as a role name.

This option is only used when the `role-mapping-mode` option has value `attribute`.

| role-context
|
| LDAP Context in which assigned roles are searched. (E.g. `ou=Roles,dc=hazelcast,dc=com`).

This option is only used when the `role-mapping-mode` option has value `reverse`.

| role-filter
| `([role-mapping-attribute]={memberDN})`
| LDAP search string which usually contains placeholder `{memberDN}` to be replaced by provided login name. (E.g. `(member={memberDN})`)
...
|
| Name of the LDAP attribute which contains either role name or role DN.

This option is  used when the `role-mapping-mode` option has value `attribute` or `direct`. If the mapping mode is `reverse` then the value is used in `role-filter` default value.

| role-mapping-mode
| `attribute`
a| Role mapping mode - it can have one of the following values:

* `attribute` - user object in the LDAP contains directly role name in the given attribute. Role name can be parsed from a DN string when `parse-dn=true`. No additional LDAP query is done to find assigned roles.
...
* `reverse` - role objects are located by executing an LDAP search query with given `role-filter`. In this case, the role object usually contains attributes with DNs of assigned users. Role search recursion can be enabled for this mode.

| role-name-attribute
|
| This option either refers to a name of LDAP attribute within role object which contains the role name in case of `direct` and `reverse` role mapping mode. Or it refers to the attribute name within X.500 name stored in `role-mapping-attribute` when `role-mapping-mode=attribute` and `parse-dn=true`.

| role-recursion-max-depth
| 1
| Sets max depth of role search recursion. The default value 1 means the role search recursion is disabled.

This option is only used when the `role-mapping-mode` option has value `direct` or `reverse`.

| role-search-scope
| `subtree`
a| LDAP search scope used for `role-filter` search. Allowed values are:
...
| user-name-attribute
| `uid`
| LDAP Attribute name which value will be used as a name in `ClusterIdentityPrincipal` added to the JAAS Subject.

| system-user-dn
|
a| Admin account DN. If configured, then following is true:

* for the user and role object search queries is used an admin connection instead of the "user" one created by LDAP bind with provided credentials;
* LDAP authentication doesn't expect full user DN to be provided as a login name. It rather expects names like `"jduke"` than `"uid=jduke,ou=Engineering,o=Hazelcast,dc=com"`;
* the admin connection allows verifying provided user credentials against a value defined in `password-attribute` option.
...
| password-attribute
|
| Credentials verification is done by new LDAP binds by default. Nevertheless, the password can be stored in a non-default LDAP attribute and in this case use `password-attribute` to configure against which LDAP attribute (within the user object) is the provided password compared during the login. As a result, if the `password-attribute` option is provided, then the extra LDAP bind to verify credentials is not done and passwords are just compared within the Hazelcast code after the retrieving user object from LDAP server.

This option is only used when the `system-user-dn` option (i.e. admin connection) is configured.

| user-context
|
| LDAP Context in which user objects are searched. (E.g. `ou=Users,dc=hazelcast,dc=com`)

This option is only used when the `system-user-dn` option (i.e. admin connection) is configured.

| user-filter
| `(uid={login})`
| LDAP search string for retrieving user objects based on provided login name. It usually contains placeholder substring `{login}` which is replaced by the provided login name.

This option is only used when the `system-user-dn` option (i.e. admin connection) is configured.

| user-search-scope
| `subtree`
a| LDAP search scope used for `user-filter` search. Allowed values are:
...
There can be also vendor or java version specific properties allowing more fine-grained control. E.g. disabling host name validation:
...
When even more control is necessary, users can implement their own `SSLSocketFactory` and use its class name as the value in `ldap` authentication option `socket-factory-class-name`.

Example custom socket factory class:
...
The authentication configuration could look like:
...
The LDAP authentication is backed  by the JNDI API in Java. It has also the failover support. Users can configure multiple space-separated URLs in the `<url>` option:
...
Hazelcast is able to protect network communication using TLS. The TLS mutual authentication is also supported. It means not only the server side identifies itself to a client side (member, client, REST client, ...), but also client side needs to prove its identity by using a TLS (X.509) certificate.

The `tls` authentication type verifies within the JAAS authentication that incoming connection already authenticated the client's TLS certificate. As a `ClusterIdentityPrincipal` is used the subject DN (distinguished name) from the client's TLS certificate.

This authentication type is able to parse a role name from the client's certificate subject DN. The `<tls>` element has an attribute `roleAttribute` which specifies part of DN to be used as a role name.
...
This `tls` authentication  uses `cn` attribute from subject DN as the role name.
If the subject DN in the certificate is `cn=admin,ou=Devs,o=Hazelcast` for instance,
then following `Principals` are added:
...
Available identity configuration types:

* `username-password` - defines a new `PasswordCredentials` object
* `token` - defines a new `TokenCredentials` object
* `credentials-factory` - configures factory class which creates `Credentials` objects
...
The content of `Credentials` object is verified during authentication.
...
There are 2 subtype interfaces which simplify `Credentials` usage.
The subtypes reflect data provided in client authentication messages:

* Name and password (`com.hazelcast.security.PasswordCredentials`)
* Byte array token (`com.hazelcast.security.TokenCredentials`)

The interfaces have following form:
...
The `Credentials` instance can be retrieved in login modules
...
A `PasswordCredentials` implementation can be configured as a simple identity representation. It's configured by XML element `<username-password/>`:
...
identity representation. The XML element `<token/>` allows using either plain
ASCII tokens or Base64 encoded values. Its optional argument `encoding` can have
value `base64` or `none` (default).

Following 2 realms defines the same token value - bytes of the "Hazelcast" string:
...
The most flexible way to define the `Credentials` objects is using a custom credential factory. It is an implementation of `com.hazelcast.security.ICredentialsFactory`
interface. Its `newCredentials()` method is the one which provides credentials.

The XML configuration uses `<credentials-factory>` element to define factory class.

Behavior of credential factories can be controlled by specifying factory properties.
The properties are provided in `init(Properties)` method.
...
Hazelcast works with 3 JAAS link:https://docs.oracle.com/javase/8/docs/api/java/security/Principal.html[Principal^] implementations added to the link:https://docs.oracle.com/javase/8/docs/api/javax/security/auth/Subject.html[Subject^]:

* `ClusterIdentityPrincipal` - represents name of the authenticated party (usually one instance in the `Subject`)
* `ClusterRolePrincipal` - represent role assigned to the authenticated party,
  (usually zero or more instances in the `Subject`)
* `ClusterEndpointPrincipal` - represents remote address of the authenticated party (usually one instance in the `Subject`)

These implementations share a common abstract parent class `HazelcastPrincipal`
so it's simple to find them in the JAAS `Subject`.
...
JAAS link:https://docs.oracle.com/javase/8/docs/api/javax/security/auth/callback/Callback.html[Callback^] instances are used for accessing different kinds of data from
link:http://docs.oracle.com/javase/8/docs/api/javax/security/auth/spi/LoginModule.html[LoginModule^] implementations. Hazelcast supports following `Callback` types:
...
* `com.hazelcast.security.CertificatesCallback` - retrieves TLS certificate chain  (if any) of the connecting party
* `com.hazelcast.security.ClusterNameCallback` - retrieves cluster name used for authentication
* `com.hazelcast.security.CredentialsCallback` - retrieves `Credentials` used for authentication
* `com.hazelcast.security.ConfigCallback` - retrieves `Config` object of current Hazelcast member
* `com.hazelcast.security.EndpointCallback` - retrieves remote address of the connecting party
* `com.hazelcast.security.SerializationServiceCallback` - retrieves `SerializationService` of current Hazelcast member


The callbacks are usually used in `login()` method of a login module:
...
shared logic and cleanup operations. It automatically creates the `ClusterEndpointPrincipal` instance and it also provides method `addRole(String)` which simplifies adding `ClusterRolePrincipal` instances.

The `ClusterLoginModule` implements all methods from `LoginModule` interface and makes them final. It provides protected methods with empty implementations (e.g. `onCommit()`) to align the logic to user needs. The module comes also with 2 abstract methods:

* `getName()` used to retrieve name of `ClusterIdentityPrincipal`
* `onLogin()` logic of the login method which needs to be provided

Extending the `ClusterLoginModule` is recommended instead of implementing all required stuff from scratch.
...
The `ClusterLoginModule` supports a basic set of login module options, which allow skipping adding principals of a given type to the JAAS Subject. It allows for instance to have just one `ClusterIdentityPrincipal` in the `Subject` even if there are more login modules in the chain:
...
authentication against the Security System of your choice, such databases, directory services or some other corporate standard you might have. For example, you may wish to have
...
be referenced by `<member-authentication/>` element to define authentication between member and identity of the current member.
...
To implement client authentication, you reference a <<security-realms, Security Realm>> with `authentication` section defined in the `<client-authentication/>` element of a cluster member configuration.
...
The identity of the connecting client is defined on client side.
There are no security realms on clients, but just identity defined directly in security configuration:
...
On clients, You can use the same identity types as in security realms:

...
+
NOTE: When you enable TLS/SSL and do not provide the `trustStore` property, the default trusted certificates are used, i.e., the Java provided list of trusted CA certificates.
+
...
Permission policy implementations can access the client permissions that are in the configuration by using the
`SecurityConfig.
getClientPermissionConfigs()` method when Hazelcast calls `configure(SecurityConfig securityConfig, Properties properties)`.
...
By default, the set of permissions defined in the leader member of a cluster is distributed to the newly joining members, overriding their own permission configurations, if any. However, you can configure a new member to be joined, so that it keeps its own set of permissions and even send these to the existing members in the cluster. This can be done dynamically, i.e., without needing to restart the cluster, using the `hazelcast.security.permissions.operation.on-join` system property. Using this property, you can choose whether a new member joining to a cluster will apply the client permissions stored in its own configuration, or use the ones defined in the cluster.

You can set the <<permission-onjoin, `hazelcast.security.permissions.operation.on-join`>> system property to the following values:

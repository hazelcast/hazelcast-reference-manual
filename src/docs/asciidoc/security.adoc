...
Detailed logging for LDAP authentication can be enabled by configuring more verbose logger  level for `com.hazelcast.security` package as described in the <<security-debugging, Security Debugging>> section.
...
The biggest part of business logic related to security in Hazelcast is located in Java package `com.hazelcast.security`. Issues can be investigated by printing more debug info from this package.

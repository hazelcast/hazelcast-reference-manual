...
Instance and principal names can be defined as wildcards using the `"*"` character.
See the <<using-wildcards, Using Wildcards>> section for details.

Endpoint names can use range characters `"-"` and `"*"` as described
in the section <<interfaces, Interfaces>>.

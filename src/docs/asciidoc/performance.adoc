...
From Hazelcast 3.12 there is a new feature called Pipelining. The idea of pipelining is that a single thread can send multiple
requests in parallel and therefor can increase throughput. Example: imagine that the round trip time for a request/response
is 1 milisecond. If synchronous requests are used (e.g. IMap.get) then the maximum throughput of doing IMap.get calls from
a single thread is 1/0001=1000 operations/second. One way to solve this problem is to have  introduce multithreading to make
requests in parallel. In the above case, if we would use 2 threads, then the maximum throughput doubles from 1000
operation/second, to 2000 operations/second.

But introducing threads for the sake of executing request isn't always convenient and doesn't always lead to optimal
performance. That is where the pipelining can be used. So instead of using multiple threads to have concurrent invocations,
you can use asynchronous method calls like IMap.getAsync. If you would use 2 asynchronous calls from a single thread,
then the maximum throughput is 2*(1/0001)=2000 operations/second. So to utilize pipelining, asynchronous calls need to
be made from a single thread. The pipelining is a convenience implementation to provides back pressure (so controls
...
---
In the above configuration we make 100 asynchronous map.getAsync calls, but the maximum number of inflight calls is 10.

By increasing the debt of the pipeline, throughput can be increased. Pipelining has its own back pressure. So you do not
need to enable back pressure on client and or member to have back pressure on the pipeline. But beware if you have many
pipelines, it could still be needed to enable client/member back pressure because it is possible to overwhelm the system
with requests in that situation.

Pipelining can be used for both clients and members. No special configuration is needed, it works out of the box.

Pipelining can be used for any asynchronous call. So you could use it for IMap asynchronous get/put, but can also be used for
e.g. ICache, IAtomicLong etc.

Pipelining can't be used as a transaction mechanism. So you can't do some calls and throw away the pipeline and expect that
non of the request get executed. If you want to use atomic behavior, please check out transactions. Pipelining is just a performance
optimization, not a mechanism for atomic behavior.

Pipelines are cheap and should frequently be replaced because they accumulate results. It is fine to have a few hundred or
even a few thousand calls being processed with the pipeline, but all responses to all the request are stored in the pipeline
as long as the pipeline is referenced. So if you want to process a huge number of request, then every few hundred or few
thousand calls, wait for the pipeline results and just create a new instance.

Pipelines are not threadsafe. They should be used by a single thread.
...
Notice that OrderKey implements `PartitionAware` and that `getPartitionKey()` returns the `customerId`. These make sure that the `Customer` entry and its ``Order``s are stored on the same member.
...
and one operation for each of the backups, i.e., `map.put` is executed for the primary and also for each backup. In most cases, there is a natural balance between the number of threads performing operations
...
This system property's default value is 100. It means, out of 100 *asynchronous* backups, Hazelcast makes 1 of them a *synchronous* one. A randomization is added, so the sync window with default configuration is between 75 and 125
invocations.
...
Refer to the <<system-properties, System Properties appendix>> for explanations of these properties.
...
After detection of an invalidation loss, stale data is made unreachable and Near Cache's `get` calls to that data are directed to the underlying IMap/JCache to fetch the fresh data.
...
The element `cache-deserialized-values` controls the caching of deserialized values. Note that caching makes the query evaluation faster, but it consumes more memory. This element has the following values:

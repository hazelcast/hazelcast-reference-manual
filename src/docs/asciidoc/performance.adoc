...
NOTE: You can also specify counts for input and output threads separately. There are `hazelcast.io.input.thread.count` and `hazelcast.io.output.thread.count` properties for this purpose. Please refer to the <<system-properties, System Properties appendix>> for information on these properties and how to set them.
...
The class https://github.com/hazelcast/hazelcast/blob/master/hazelcast/src/main/java/com/hazelcast/config/NearCacheConfig.java[NearCacheConfig] is used for all supported Hazelcast data structures on members and clients.

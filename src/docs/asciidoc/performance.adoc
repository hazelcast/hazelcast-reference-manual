...
===== Non Operations that are Partition-aware
...
The `SlowOperationDetector` stores the fully qualified classname of the operation and its stacktrace as well as operation details, start time and duration of each slow invocation. All collected data is available in the <<management-center, Management Center>>.

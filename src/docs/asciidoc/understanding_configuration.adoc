...
The above example using YAML configuration files looks the following:
...
Resources from classpath and file system may also be used for composing declarative configuration:
...
Importing resources with variables in their names is also supported. Please see the following example snippets:
...
Starting with 3.12 Hazelcast members and Java clients can be configured declaratively with YAML configuration files in
installations of Hazelcast running on Java runtime version 8 or above.

The structure of the YAML configuration follows the structure of the XML configuration making rewriting existing XML
configurations to YAML easier. There are some differences between the XML and the YAML languages that make the two declarative configurations to slightly derive as the the following examples show.
...
kind of configurations is listing member addresses, interfaces in the networking configurations, defining listeners. The
...
In the YAML configuraion the listeners are defined as a sequence.
...
the other configuration of the multicast configuration.
...
[[configuring-declaratively-precedence]]
==== Precedence of Declarative Configuration Files
...
Building `Config` from XML declarative configuration.
...
Building `Config` from YAML declarative configuration.
...
configuration files is parsed as an
...
refer to `ConfigReplacer` s https://docs.hazelcast.org/docs/latest/javadoc/com/hazelcast/config/replacer/spi/ConfigReplacer.html[Javadoc] for basic information on how a replacer works.

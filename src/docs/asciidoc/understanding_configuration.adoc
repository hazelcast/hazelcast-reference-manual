...
Whenever you want to override the configuration,
you can achieve this by using system properties or environment variables.

For example, if you just want to override a `cluster-name`, you'd need to craft an XML/YAML configuration file:
[source,xml]
----
<hazelcast>
...
The same could be achieved by setting a single environment variable:
[source]
----
...
NOTE: The above mechanism exists mostly for the sake of trivial environment-specific configuration changes
and should not be treated as replacement for XML/YAML config files.

==== Conversion Rules

In order to make Hazelcast recognize environment variables as valid config entries, they need to obey a few simple rules:

* Each config entry needs to start with `HZ_` or `HZ_CLIENT_`
...
Would be represented as a set of following environment variables:
[source]
----
export HZ_CLUSTERNAME=dev
export HZ_NETWORK_PORT_AUTOINCREMENT=true
export HZ_NETWORK_PORT_PORTCOUNT=100
export HZ_NETWORK_PORT_PORT=5701
export HZ_NETWORK_JOIN_AUTODETECTION_ENABLED=true
----

In order to make Hazelcast recognize system properties as valid config entries, they need to obey a few simple rules:

* Each config entry needs to start with `hz.` or `hz-client.`
* A new configuration level should be introduced with an underscore (`_`)
...
Would be represented as a set of following system properties:
[source]
----
hz.cluster-name=dev
...
In both cases, the entry structure needs to follow Hazelcast YAML configuration representation.

WARNING: Keep in mind that it's not possible to represent configuration entries represented as YAML sequences.

...
[source,plain,indent=0,subs="verbatim,attributes",role="primary"]
...
as a replacement for the XML/YAML configuration files.
...
In both cases, the entry structure needs to follow the
Hazelcast YAML configuration representation.

WARNING: Keep in mind that it's not possible to represent
configuration entries represented as YAML sequences.

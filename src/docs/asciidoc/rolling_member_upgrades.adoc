...
Hazelcast members operating on binaries of the same major and minor version numbers are compatible regardless of patch version.
  For example, in a cluster with members running on version 3.11.1, it is possible to perform a rolling upgrade to 3.11.2 by shutting down, upgrading to `hazelcast-3.11.2.jar` binary
...
Also, each minor version is compatible with the previous one (back until Hazelcast IMDG 3.8). For example, it is possible to perform a rolling upgrade on a cluster running Hazelcast IMDG Enterprise 3.11 to Hazelcast IMDG Enterprise 3.12.
...
* Shutdown gracefully an existing `3.12.0` member.
...
** `hazelcast.cluster.version.auto.upgrade.enabled`: Specifies whether the automatic cluster version upgrading is enabled. Its default value is `false` (disabled).
** `hazelcast.cluster.version.auto.upgrade.min.cluster.size`: When this property is set to a value greater than 1, automatic upgrading waits to reach that cluster size to proceed. Its default value is `1`.

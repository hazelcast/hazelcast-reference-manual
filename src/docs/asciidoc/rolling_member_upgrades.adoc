...
* `hazelcast.cluster.version.auto.upgrade.enabled`: Default value is false. Set this property to true to enable cluster version auto upgrade.
* `hazelcast.cluster.version.auto.upgrade.min.cluster.size`: Default value is 1. When this property is set greater than 1, auto upgrade waits to reach that cluster size to proceed.

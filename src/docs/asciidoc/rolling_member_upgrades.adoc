...
version, it will upgrade the cluster version to match it. This feature is
disabled by default and to enable it, set the system property 
`hazelcast.cluster.version.auto.upgrade.enabled` to `true`.

There is one tricky detail here: as you're shutting down and upgrading 
...
member as well. To avoid this we provide another system property: 
`hazelcast.cluster.version.auto.upgrade.min.cluster.size`. You should 

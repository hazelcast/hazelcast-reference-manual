...
NOTE: Starting with Hazelcast 3.9, if you configure the data structure to use <<configuring-high-density-memory-store, High-Density Memory Store>> **and** indexes, the indexes are automatically stored in the High-Density Memory Store as well. This prevents from running into full garbage collections, when doing a lot of updates to index.

===== Composite Indexes

Composite indexes, also known as compound indexes, are special kind of indexes that are built on top of multiple map entry attributes and therefore may be used to significantly speed up queries involving those attributes simultaneously.
...
Unordered indexes are used to perform equality queries, also known as point queries, e.g. `name = 'Alice'`. Unordered indexes are specifically optimized for equality queries and don't support other comparison operators like `>` or `+<=+`.

Additionally, _composite_ unordered indexes allow to speed up equality queries involving multiple attributes simultaneously, e.g. `name = 'Alice' and age = 33`. The latter query will result in a single composite index lookup operation which can be performed very efficiently.

Unordered composite index on attributes `name` and `age` may be configured for a map as follows:
...
Attributes indexed by _unordered_ composite indexes can't be matched partially: `name = 'Alice'` query can't utilize the composite index configured above.

====== Ordered Composite Indexes

Ordered indexes are specifically designed to perform efficient order comparison queries, also known as range queries, e.g. `age > 33`. Equality queries, like `age = 33`, are still supported by ordered indexes, but they are handled in a slightly less efficient manner comparing to unordered indexes.

_Composite_ ordered indexes extend the concept by allowing multiple equality predicates and a single order comparison predicate to be combined into a single index query operation. For instance, `name = 'Alice' and age > 33` and `name = 'Bob' and age = 33 and balance > 0.0` queries are good candidates to be covered by an ordered composite index configured as follows:
...
Attributes involved in a query can be matched partially by the composite index matcher: `name = 'Alice' and age = 33 and balance > 0.0` can be partially matched by `name, age` composite index, the `name = 'Alice' and age = 33` predicates will be served by the matched index, while the `balance > 0.0` predicate will be processed by other means.

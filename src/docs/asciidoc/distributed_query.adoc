...
which offers simpler predicate building. Please see the
...
keys. Please see the following example:
...
Starting with Hazelcast 3.6, you can also access a specific page more
...
Please note that this feature tires the memory and refer to the
...
inserted, thereby controlling the owning partition. Please see the
...
`ValueExtractor`. Please see the <<custom-attributes, Custom Attributes section>>
for details.
...
Please refer to the https://docs.hazelcast.org/docs/latest/javadoc/com/hazelcast/query/extractor/ValueExtractor.html[`ValueExtractor`] and
...
for map aggregations and general data processing, respectively. Please see the
...
size or on what keys the operation will operate. For more information, please refer to
the https://docs.hazelcast.org/docs/latest/javadoc/com/hazelcast/mapreduce/Job.html[Hazelcast source code for Job.java].
...
You can refer to <<fast-aggregations, Fast Aggregations>> for examples. If you
...
configure the MapReduce framework, please see <<configuring-jobtracker, Configuring JobTracker>>.
We will
...
your aggregation operations based on MapReduce calls. For
more information, refer to the
<<implementing-aggregations, Implementing Aggregations section>>.
...
Each phase is described below and you can also refer to the
...
with or without a predicate. You can refer to its
...
Please see the <<registering-map-listeners, Registering Map Listeners section>>.
* `in-memory-format`: Type of the data to be stored in your query cache.
Please see the <<setting-in-memory-format, Setting In-Memory Format section>>.
...
* `eviction`: Configuration for the eviction of your query cache. Please see the
...
sub-elements. Please see the <<configuring-imap-indexes, Configuring IMap Indexes section>>.

...
you first need to create a `HazelcastJsonValue` from the json string. You can use `HazelcastJsonValue`s as both key and value in distributed data structures. Then, it is
...
whether such entry is going to be returned or not from a query is not defined.

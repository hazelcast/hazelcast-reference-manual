...
NOTE: See the
https://docs.hazelcast.org/docs/latest/javadoc/com/hazelcast/query/Predicates.html[Predicates Javadoc]
...
https://docs.hazelcast.org/docs/latest/javadoc/com/hazelcast/query/PagingPredicate.html[PagingPredicate Javadoc].
...
A concrete example may be a webshop that sells phones and accessories.
...
The same set of predicates is supported: `equal`, `notEqual`, `in`,
`and`, `or` and `not`. But, unlike hash indexes, bitmap indexes are able
to achieve a much higher memory efficiency at the cost of reduced query
performance. In practice, the query performance is comparable to the
performance of hash indexes, while memory footprint reduction is high,
usually around an order of magnitude.

Bitmap indexes are especially useful for indexing of collection and
...
Internally, a unique non-negative `long` ID will be assigned to every
indexed `IMap` entry based on the entry key. That unique `long` ID is
...
on `age` attribute, extract unique key values from `uniqueId` attribute
...
that causes increased memory usage in bitmap indexes since the best case
...
The index definition above instructs the bitmap index to extract unique
...
Where `<attr>` specifies the attribute index, `<key>` specifies the
attribute to use as a unique key source for internal unique `long` ID
assignment and `<transform>` specifies the transformation to be applied
to unique keys to generate unique `long` IDs from them. The following
transformations are supported:

    - `OBJECT`: object-to-long transformation. Each extracted unique key
      value is interpreted as a Java object instance. Internally, an
      object-to-long hash table is used to establish the mapping from
      unique keys to unique IDs. Good as a general-purpose transformation.

    - `LONG`: long-to-long transformation. Each extracted unique key
      value is interpreted as a non-negative `long` value, the widening
      conversion from `byte`, `short` and `int` is performed, if
      necessary. Internally, a long-to-long hash table is used to
      establish the mapping from unique keys to unique IDs, which is
      more efficient than the object-to-long hash table. Good for
      sparse/random unique integer-valued keys renumbering to raise the
      IDs density and to make the bitmap index more memory-efficient as
      a result.

    - `RAW`: raw transformation. Each extracted unique key value is
      interpreted as a non-negative `long` value, the widening
      conversion from `byte`, `short` and `int` is performed, if
      necessary. Internally, no hash table of any kind is used to
      establish the mapping from unique keys to unique IDs, the raw
      extracted keys are used directly as IDs. Good for dense unique
      integer-valued keys, has the best performance in terms of time and
      memory.

The regular doted attribute path syntax is supported for `<attr>` and
`<key>`:
...
See <<Indexing in Collections and Arrays>> for more details.

====== Bitmap Index Querying

Bitmap index matching and selection for queries is performed
...
See the https://docs.hazelcast.org/docs/latest/javadoc/com/hazelcast/query/extractor/ValueExtractor.html[`ValueExtractor`] and
https://docs.hazelcast.org/docs/latest/javadoc/com/hazelcast/query/extractor/ValueCollector.html[`ValueCollector`] Javadocs.
...
See the https://docs.hazelcast.org/docs/latest/javadoc/com/hazelcast/query/extractor/ValueReader.html[`ValueReader`] Javadoc.
...
<<fast-aggregations, Fast-Aggregations>> and https://jet.hazelcast.org/[Hazelcast Jet]
...
the https://docs.hazelcast.org/docs/latest/javadoc/com/hazelcast/mapreduce/Job.html[Job.java] for more information.
...
See the <<fast-aggregations, Fast Aggregations section>> for examples. If you
...
NOTE: This feature has been deprecated. Please use the <<fast-aggregations, Fast-Aggregations>>
instead.
...
configure the MapReduce framework, see the <<configuring-jobtracker, Configuring JobTracker section>>.
We will
...
(as described in <<retrieving-a-jobtracker-instance, Retrieving a JobTracker Instance>>)
...
<<data-affinity, Data Affinity>> rules to influence the partitioning of data.
...
Fast-Aggregations always run on the partition threads. If the data is of type `BINARY`
...
https://docs.hazelcast.org/docs/latest/javadoc/com/hazelcast/aggregation/Aggregator.html[Aggregator Javadoc]
...
Since Fast-Aggregation is executed in parallel on each partition of the cluster,
...
to its https://docs.hazelcast.org/docs/latest/javadoc/com/hazelcast/core/IMap.html#aggregate-com.hazelcast.aggregation.Aggregator-[Javadoc]
...
to the https://docs.hazelcast.org/docs/latest/javadoc/com/hazelcast/projection/Projection.html[Projection Javadoc]
for the API's details.
...
https://docs.hazelcast.org/docs/latest/javadoc/com/hazelcast/core/IMap.html#project-com.hazelcast.projection.Projection-[Javadoc]
to see the method details.

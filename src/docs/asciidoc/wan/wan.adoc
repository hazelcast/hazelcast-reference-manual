...
* `custom-publisher`: Custom implementation of a WAN publisher implementing `WanPublisher`.
...
`WanPublisher`. For example, you might implement replication to Kafka or some JMS queue or even
...
* `class-name`: Name of the class implementing a custom WAN consumer (`WanConsumer`).

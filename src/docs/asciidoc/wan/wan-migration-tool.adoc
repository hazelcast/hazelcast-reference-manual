=== WAN Migration Tool for 3.12 and 4.0/4.1 Releases

This section outlines a solution for migrating user data from a
running 3.12 cluster to a 4.0 or 4.1 cluster and vice versa using WAN replication.
...
available as non-public releases, which are able to accept connections and packets
...
a minimal amount of compatibility code. To be exact, these are public Hazelcast
releases from 3.12.8 and from 4.0.2 onward. The "plain" members are not able to
...
1. The first step is to setup the 3.12 cluster. You can also use an existing 3.12 cluster.
The members of the 3.12 cluster should have a minor version of at least 3.12, but it can be of any
patch level. You need to setup WAN replication between the 3.12 and 4.0/4.1 clusters but this will
be described soon.
2. Next, setup a 4.0/4.1 cluster. It must have at least one "migration" member and
all other ("plain") members should have a patch version of at least 4.0.2.
...
The members of the 4.0/4.1 cluster can be of any patch level. You need to set up WAN replication
...
("plain") members should have a patch version of at least 3.12.8. The "migration" member
can also be a "lite" member while the other "plain" members need not be. Making the "migration"
member a "lite" member will also simplify further migration steps.
3. Set up WAN replication between the two clusters. As always, this can be done using the static
...
at least one "migration" member. The "migration" member can also be a "lite" member while the other
"plain" members need not be. Making the "migration" member a "lite" member will also simplify
further migration steps. Other "plain" members of the 3.12 cluster must have a patch version of
at least 3.12.8 and members of the 4.0/4.1 cluster must have a patch version of at least 4.0.2.
2. Setup WAN replication between the two clusters. As always, this can be done using the static configuration
...
but with the added restriction that they must be at least 3.12.8 (in case of a 3.12 cluster)
or 4.0.2 (in case of a 4.0/4.1 cluster).

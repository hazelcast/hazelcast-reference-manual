...
Caches are generally not expected to grow to an infinite size. Implementing an <<defining-a-custom-expirypolicy, expiry policy>> is one way you can prevent infinite growth, but sometimes it is hard to define a meaningful expiration timeout. Therefore, Hazelcast JCache provides the eviction feature. Eviction offers the possibility of removing entries based on the cache size or amount of used memory

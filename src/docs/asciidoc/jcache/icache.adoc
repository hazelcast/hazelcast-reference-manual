...
In general, the `String getName()` method over `HazelcastInstance` is safer and
...
Hazelcast instances by that instance name, respectively.
...
When a valid instance name is provided, the property value is used to resolve the `HazelcastInstance`
the first time a `CacheManager` is created.
...
When an existing `HazelcastInstance` object is available, you can pass
it to `CacheManager`.
...
See the link:{docBaseUrl}/javadoc/com/hazelcast/cache/ICache.html[ICache Javadoc^]

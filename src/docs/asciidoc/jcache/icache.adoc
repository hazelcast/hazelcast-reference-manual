...
link:https://jet.hazelcast.org/[Hazelcast Jet^] for Real-Time Stream Processing
...
(writes data to ICache). See the link:https://jet.hazelcast.org/use-cases/fast-batch-processing/[Fast Batch Processing^]
...
link:https://docs.hazelcast.org/docs/jet/latest/manual/index.html#connector-imdg[here^] in the
...
instance and is auto-generated by using group name (if it is specified).
...
* `split-brain-protection-ref` : Name of split brain protection configuration that you want this cache to use.
...
all methods, returning a `com.hazelcast.core.ICompletableFuture`. By using these
methods and the returned future objects, you can use JCache in a reactive way by
...
You can specify the full class name of custom `EvictionPolicyComparator`
(`CacheEvictionPolicyComparator` for JCache and its Near Cache) implementation 
in the `<eviction>` tag through `comparator-class-name` attribute in Hazelcast
...
(`CacheEvictionPolicyComparator` for JCache and its Near Cache) bean in the `<eviction>` tag 
...
can be downloaded from link:http://download.oracle.com/otndocs/jcp/jcache-1_0-fr-eval-spec/index.html[here^])
...
See the link:https://docs.hazelcast.org/docs/latest/javadoc/com/hazelcast/cache/ICache.html[ICache Javadoc^]

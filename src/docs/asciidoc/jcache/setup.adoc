...
available in the classpath. This way, the Hazelcast implementation of a `CachingProvider` automatically starts a new Hazelcast
...
Note that this section only describes the JCache provided standard properties. For the Hazelcast specific properties, see the <<icache-configuration, ICache Configuration section>>.

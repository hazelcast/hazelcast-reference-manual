...
of a `CachingProvider` automatically and by default starts a new Hazelcast
client and pick up the configuration from either the command line parameter
...
The next line in the example code above asks the `CachingProvider` to return a

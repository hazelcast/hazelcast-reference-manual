...
<hazelcast>
    ...
    <wan-replication name="my-wan-cluster-batch">
        <wan-publisher cluster-name="london">
            ...
            <properties>
                ...
                <property name="cluster.password">london-pass</property>
...
* `cluster-name`: Specifies the target cluster's name for authentication on the target endpoint.
It is mandatory to set this attribute.
* `publisher-id`: Specifies the publisher ID used for identifying the publisher.
Setting this ID may be useful when the target cluster names are not unique for all of
...
* `cluster.password`: Configures target cluster's password.
...
<hazelcast>
    ...
    <wan-replication name="my-wan-cluster">
        <wan-publisher cluster-name="test-cluster-1">
...
<hazelcast>
    ...
    <wan-replication name="my-wan-cluster">
        <wan-publisher cluster-name="istanbul">
            <class-name>com.hazelcast.enterprise.wan.replication.WanBatchReplication</class-name>
        </wan-publisher>
    <wan-replication>
    <map name="my-map">
...
<hazelcast>
    ...
    <wan-replication name="wanReplicationScheme">
        <wan-publisher cluster-name="clusterName">

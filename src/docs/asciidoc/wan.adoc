...
```
curl -X POST -d "clusterName&clusterPassword&my-wan-cluster&istanbul&my-map" --URL http://127.0.0.1:5701/hazelcast/rest/mancenter/wan/sync/map
```
...
```
curl -X POST -d "clusterName&clusterPassword&{...}" --URL http://127.0.0.1:5701/hazelcast/rest/mancenter/wan/addWanConfig
...
`+http://member_ip:port/hazelcast/rest/mancenter/wan/sync/allmaps+`. You need to add the following URL-encoded parameters to
the request in the following order separated by "&";

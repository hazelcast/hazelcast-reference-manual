...
* `topic.base.name`: Base topic name to use while publishing events to Solace appliance. If not defined, the members publish to topics using the format "T/hz/[cluster-group-name]/partitionId". If defined, the format "[topic.base.name]/partitionId" is used. It is an optional property.
...
* `persist-wan-replicated-data`:  When true, an incoming event over WAN replication can be persisted to a data store, otherwise it is not persisted. Default value is false.
* `host`: IP address of the Solace host machine. It can be in the format "IP address:Port number". It is a mandatory property.
* `vpn.name`: Name of the Solace VPN. It is an optional property.
* `username`: Username for the Solace host. It is a mandatory property.
* `password`: Password for the Solace host. It is an optional property.
...
You can find the Hazelcast Solace code samples https://github.com/hazelcast/hazelcast-code-samples/tree/master/enterprise/wan-replication/src/main/java/com/hazelcast/wan/solace[here].

You can also download the white paper **Hazelcast WAN Replication with Solace** from https://hazelcast.com/resources/hazelcast-wan-replication-solace/[Hazelcast.com].

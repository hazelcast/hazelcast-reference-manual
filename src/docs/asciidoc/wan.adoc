...
When running clusters for an extensive period you might need to dynamically change configuration while the cluster is running. This includes dynamically adding new WAN replication publishers (new target clusters) and have subsequent map and cache updates be replicated to the new publishers without any manual intervention.

You can add new WAN publishers to an existing WAN replication using almost all of the configuration options that are available when configuring WAN publishers in the static configuration (including using discovery SPI). The new configuration is not persisted but it is replicated to all existing and new members. Once the cluster is completely restarted, the dynamically added publisher configuration is lost and the updates are not replicated to the target cluster anymore until added again.
...
Existing configurations cannot be removed but publishers can be put into a STOPPED state which prevents WAN events from being enqueued in the WAN queues and prevents replication, rendering the publisher idle. Configurations also cannot be changed.
...
Note that the map has defined WAN replication but there is no target cluster yet. The user can then add new WAN replication publishers (target clusters) by performing a HTTP POST:
...
The user provides the full configuration as JSON as a parameter. Any WAN configuration supported in XML and Java config is also supported in this JSON format. Below are some examples of JSON configuration for a WAN publisher using Discovery SPI and static IP configuration. The integer values for "initialPublisherState", "queueFullBehavior" and "consistencyCheckStrategy" can be found in the javadoc for the respective java configuration classes ("com.hazelcast.config.WanPublisherState","com.hazelcast.config.WANQueueFullBehavior" and "com.hazelcast.config.ConsistencyCheckStrategy").

Example with Discovery SPI config (AWS config):
```
{
   "name":"wanReplication",
   "publishers":[
...
Example with Discovery SPI (new AWS config)
```
{
   "name":"wanReplication",
   "publishers":[
...
Example with static IP config (with some optional attributes):
```
{
   "name":"wanReplication",
   "publishers":[
...
XML config with two publishers and several (disabled) discovery strategy configurations:
```
{
   "name":"wanReplication",
   "publishers":[

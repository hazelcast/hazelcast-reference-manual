...
the link:http://download.oracle.com/javase/1.5.0/docs/guide/management/agent.html[JMX agent^]:
...
It's possible to restrict source IP addresses from which Management center operations
are allowed. By default all source connections are allowed.

Defining source addresses is possible through the property `hazelcast.mc.trusted.interfaces`. The wildcard (`*`) and ranges can be used.
...
NOTE: The property `hazelcast.mc.trusted.interfaces` is only available in patch releases for Hazelcast IMDG minor version `4.0`. Starting with IMDG `4.1` new configuration element is added under `<management-center />` configuration and the property-based configuration is not supported anymore.

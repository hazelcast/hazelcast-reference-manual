...
To compute an aggregated value of `getAverageHitSelectivity()` for all cluster members one may use a simple averaging computation: `(s(1) + s(2) + ... + s(n)) / n` where `s(i)` is an average hit selectivity on the member `i` and `n` is the total number of cluster members. A more advanced solution is to compute a weighted average: `(s(1) * h(1) + s(2) * h(2) + ... + s(n) * h(n)) / (h(1) + h(2) + ... + h(n))` where `s(i)` is an average hit selectivity on the member `i`, `h(i)` is a hit count (`getHitCount()`) on the member `i` and `n` is the total number of cluster members. The later method may produce more precise results in unstable dynamic clusters where new members don't have enough statistics accumulated. The same technique may be applied to the `getAverageHitLatency()` metric.

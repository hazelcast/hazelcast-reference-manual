...
This plugin samples the operation threads and checks the running operations/tasks. Hazelcast has the <<slowoperationdetector, slow operation detector>> which is useful for identifying slow operations. In other scenarios you may wish to find high volume operations. The OperationThreadSamples plugin improves visibility into which operations are running.

The OperationThreadSamples plugin is configured using the following properties:

* `hazelcast.diagnostics.operationthreadsamples.period.seconds`: The frequency that collected information is written to disk. An efficient value for production would be 30, 60, or more seconds. 0, which is the default value, disables the plugin.
* `hazelcast.diagnostics.operationthreadsamples.sampler.period.millis`: The period in milliseconds between samples. Lower values increase precision at the cost of higher overhead. The default value is 100 ms.  
* `hazelcast.diagnostics.operationthreadsamples.includeName`: Specifies whether the data structures' name pointed to by the operation (if available) should be included in the name of the samples. The default value is false.

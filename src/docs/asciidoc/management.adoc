...
Support for script execution is enabled in Hazelcast and disabled in Hazelcast Enterprise by default. The reason is security. Script engines allow to access underlying system on members (files and other resources). Scripts get access to the system on which member is running with permissions of the running user.

The scripting can be allowed or forbidden by specifying `hazelcast.mancenter.scripting.enabled` Hazelcast property within Hazelcast member configuration file:
...
NOTE: The property only works in patch releases for Hazelcast 3.11 (starting 3.11.1). The feature is controlled by the `scripting-enabled` attribute in `<management-center/>` element from version 3.12 onward.

...
* _Checking if a node is ready to use:_
+
When a node is joined/formed a cluster and user can safely use it, the HTTP call below returns status code 200, otherwise it returns status code 503 to indicate node is not available yet.
+
[source,plain]
----

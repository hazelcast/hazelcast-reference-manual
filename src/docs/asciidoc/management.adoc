...
To achieve this, first add the following system properties to
enable the http://download.oracle.com/javase/1.5.0/docs/guide/management/agent.html[JMX agent]:
...
Then enable JMX by setting the `hazelcast.jmx` property to `true` using one of the following ways:
...
|`-g` or `--groupname`
|`dev`
|Defines the name of a cluster group which is used for a simple authentication.
See the <<creating-cluster-groups, Creating Cluster Groups section>>.

|`-P` or `--password`
|`dev-pass`
|Defines the password of a cluster group (valid only for Hazelcast releases older than 3.8.2).
See the <<creating-cluster-groups, Creating Cluster Groups section>>.

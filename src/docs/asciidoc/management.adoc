...
Defining the source addresses is possible through the
`hazelcast.mc.trusted.interfaces` property. The wildcard (`*`) and ranges can be used.
...
NOTE: This property is only available in the patch releases for Hazelcast IMDG minor version `4.0`.
Starting with IMDG `4.1` a new configuration element is added under `management-center` configuration
block and the property-based configuration is not supported anymore.

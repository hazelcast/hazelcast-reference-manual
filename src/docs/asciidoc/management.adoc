...
* Memory cost of an index (`getMemoryCost()`). For on-heap stoarges this memory cost metric value is a best-effort approximation and doesn't indicate a precise on-heap memory usage of an index.

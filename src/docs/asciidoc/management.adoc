...
It samples the operation threads to see what they are doing. This is a pretty low overhead plugin.

You can configure OperationThreadSamples diagnostics using:

* `hazelcast.diagnostics.operationthreadsamples.period.seconds`: It default value is `0` meaning that the plugin is disabled.
* `hazelcast.diagnostics.operationthreadsamples.sampler.period.millis`: The period between taking samples. Defaults to 100. The lower the value, the higher the precision but also the higher the overhead.
* `hazelcast.diagnostics.operationthreadsamples.includeName`: If the name of the data structure should be included. Defaults to false. This feature should not be used in production because it can lead to memory leaks when data-structures get destroyed. Their samples will always remain.

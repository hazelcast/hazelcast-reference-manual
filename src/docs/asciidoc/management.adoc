...
The scripting can be allowed or prevented by specifying the `hazelcast.mancenter.scripting.enabled` system property within Hazelcast the member configuration file, under the `properties` element:
...
NOTE: This property only works in patch releases for Hazelcast IMDG 3.11 (starting with 3.11.1). The feature is controlled by the `scripting-enabled` attribute of the `management-center` element from Hazelcast IMDG 3.12 onward.

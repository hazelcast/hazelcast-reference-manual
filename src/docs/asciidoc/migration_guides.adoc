...
4.0 is a major version release.  The last major version release was over 5 years ago.
Major releases allow us to break compatibility in wire protocols and API, as well as removing
previously deprecated API.

As breaking changes have been made to client and cluster member protocols it will not be possible
to perform any in-place or rolling upgrade from a running 3.x cluster to 4.x.  The only way to upgrade
to 4.x is to completely shutdown the cluster.
...
==== Changing the `IMap` Eviction Configuration

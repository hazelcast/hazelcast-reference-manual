...
legacy implementations of distributed concurrency APIs, e.g. `ILock`, `IAtomicLong` etc., had been deprecated.
In 4.0, these deprecated implementations and additionally `ILock` and `ICondition` interfaces
are completely removed.

Differently from Hazelcast IMDG 3.12, in 4.0 _CP Subsystem_ received an _unsafe_ operation mode
which provides weaker consistency guarantees similar to former implementations in Hazelcast IMDG 3.x series.

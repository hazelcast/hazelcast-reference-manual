...
Deprecated `getXaResource()` method is removed, `HazelcastInstance.getXAResource()`
should be used instead.

==== Removal of Deprecations in `DistributedObjectEvent`

Deprecated `getObjectId()` method is removed, `getObjectName()` should be used
instead.

==== Removal of Deprecated `EntryListener`-based Listener API in `IMap`

The following set of deprecated `EntryListener`-based listener API methods was
removed:
...
A new performance feature in Hazelcast 4.0 is the write through optimization. This will help to reduce latency in case
of single threaded usages. Normally when a request is made, the request is handed over the the IO system where an
IO thread will take care of sending it over the wire. This is great for throughput, but in case of single threaded setups
it adds to the latency and therefor it reduced throughput because threads need to be notified.

In Hazelcast 4.0 we detect single threaded usage and will try to write through to the socket directly instead of handing
it over to the IO thread; we call this the write through.  This technique is being applied on the client, but also on the server.
We have something similar when responses are received; normally a response is processed by the response thread; but in case of
single threaded usage, the response is processed on the IO thread so we can remove a thread notification and therefor get
higher throughput.

Both the write through and response through are enabled by default. If we detect that there are many threads active, the
response and write through are disabled so it won't cause a performance degradation.
...
==== Quorum Renamed as Split Brain Protection
...
[source,xml]
----
<wan-publisher cluster-name="nyc" publisher-id="nycPublisherId">
    ...
    <properties>
        ...
        <property name="cluster.password">nyc-pass</property>
    </properties>
</wan-publisher>
----
...
in Hazelcast IMDG 3.12. Hazelcast IMDG 3.11.x checks cluster name and password, while 3.12 checks
just the cluster name when security is disabled, and it uses the client login modules when the security is enabled.

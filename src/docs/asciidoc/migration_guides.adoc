...
All legacy merge policies were removed. Replacements of legacies are under `com.hazelcast.spi.merge` package.
...
Programmatic:
`setMergePolicy/getMergePolicy` methods were removed from MapConfig, ReplicatedMapConfig and CacheConfig in favor of `setMergePolicyConfig/getMergePolicyConfig`.

Declarative:
Same <merge-policy/> tag with older versions can still be used: `<merge-policy batch-size="100">LatestAccessMergePolicy</merge-policy>`

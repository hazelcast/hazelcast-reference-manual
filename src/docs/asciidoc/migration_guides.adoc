...
====== Group configuration replaced by simple cluster name configuration

Both the client and member configuration have the `GroupConfig` (or `<group>` in XML) replaced
...
Hazelcast 4 replaces the `<member-credentials-factory>`, `<member-login-modules>` and
...
It define security configuration independently on the configuration
part where the security will be used. The component requesting security just references
...
The `<credentials>` configuration is not supported anymore in the client security configuration.
Existing `<credentials-factory>` configuration allows to fully replace the credentials as it is more flexible.
There are also new `<username-password>` and `<token>` configuration elements
which simplify the migration.

...
as you can see in the interface link:{docBaseUrl}/javadoc/com/hazelcast/core/MapStore.html[code^].
...
===== Lifecycle of a MapLoader

When created on a member, `MapLoader` can be used for operations
such as establishing a connection to the database or
accessing to other Hazelcast maps.

Since those operations access to a Hazelcast instance, a `Maploader`
lifecycle is needed, which has the create, use and destroy stages.
While `MapLoader` is handling the "use" stage, `MapLoaderLifecycleSupport`
provides the "create" and "destroy" stages.

The `MapLoaderLifecycleSupport` interface provides this lifecycle support for
implementing the `MapLoader` class. This interface has the `init()` and
`destroy()` methods.

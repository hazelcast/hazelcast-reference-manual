...
==== Metadata Policy
IMap offer automatic preprocessing of some data types at update time in order to make queries faster. It is
currently supported by only <<querying-json-strings, HazelcastJsonValue>> type. When metadata creation is on,
IMap creates additional metadata about objects of supported types and use this metadata during querying.
It does not affect latency and throughput of object of any type other than
supported types.

This feature is on by default. You can configure it via metadata-policy config.

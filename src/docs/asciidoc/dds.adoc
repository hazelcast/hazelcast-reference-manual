...
When you create a Reliable Topic, Hazelcast automatically creates a Ringbuffer for it. The name for this Ringbuffer will be the name of Reliable Topic prefixed with `_hz_rb_`. For example, if you have a Reliable Topic with the name "sometopic", the Ringbuffer backing it will have the name `_hz_rb_sometopic`. So, when you configure this Ringbuffer for the Reliable Topic, you will use this prefixed name. By default, a Ringbuffer does not have any TTL (time to live) and it has a limited capacity; you may want to change that configuration. The following is an example configuration for the "sometopic" given above.
...
<hazelcast>
    ...
    <ringbuffer name="_hz_rb_sometopic">

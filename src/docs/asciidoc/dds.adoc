...
NOTE: Please note that some of the partitions do not contain any data entries since we only have 120 objects and the partition count is 271 by default. This count is configurable and can be changed using the system property `hazelcast.partition.count`. Please see the <<system-properties, System Properties appendix>>.
...
NOTE: As of Hazelcast 3.7, the elements `eviction-percentage` and `min-eviction-check-millis` are deprecated. They are ignored if configured since map eviction is based on the sampling of entries. Please see the <<eviction-algorithm, Eviction Algorithm section>> for details.
...
`FencedLock` is a linearizable & distributed implementation of `java.util.concurrent.locks.Lock`, meaning that if you lock using an FencedLock, the critical
...
NOTE: In 3.12, `ILock` interface and implementation of `ILock` has been deprecated. To read about `ILock`, please refer to https://docs.hazelcast.org/docs/3.11.2/manual/html-single/index.html#lock[Lock] section in 3.11. This section will introduce `FencedLock` provided by *CP Subsystem*. For more info see <<cp-subsystem, CP Subsystem>>.
...
NOTE: For detailed information and configuration please refer to <<fencedlock, FencedLock>> under CP Subsystem section.
...
NOTE: In 3.12, the original implementation of IAtomicLong has been deprecated. To read about previous implementation, please refer to https://docs.hazelcast.org/docs/3.11.2/manual/html-single/index.html#iatomiclong[IAtomicLong] section in 3.11. This section will introduce `IAtomicLong` provided by *CP Subsystem*. For more info see <<cp-subsystem, CP Subsystem>>.
...
NOTE: In 3.12, the original implementation of ISemaphore has been deprecated. To read about previous implementation, please refer to https://docs.hazelcast.org/docs/3.11.2/manual/html-single/index.html#isemaphore[ISemaphore] section in 3.11. This section will introduce `ISemaphore` provided by *CP Subsystem*. For more info see <<cp-subsystem, CP Subsystem>>.
...
NOTE: ISemaphore with a single permit may be considered as a lock. Unlike the locks, when semaphores are used, any thread can release the permit depending on the configuration, and semaphores can have multiple permits. For more info please refer to <<semaphore-configuration, Semaphore Configuration>> section.
...
NOTE: In 3.12, the original implementation of IAtomicReference has been deprecated. To read about previous implementation, please refer to https://docs.hazelcast.org/docs/3.11.2/manual/html-single/index.html#iatomicreference[IAtomicReference] section in 3.11. This section will introduce `IAtomicReference` provided by *CP Subsystem*. For more info see <<cp-subsystem, CP Subsystem>>.
...
NOTE: In 3.12, the original implementation of ICountDownLatch has been deprecated. To read about previous implementation, please refer to https://docs.hazelcast.org/docs/3.11.2/manual/html-single/index.html#icountdownlatch[ICountDownLatch] section in 3.11. This section will introduce `ICountDownLatch` provided by *CP Subsystem*. For more info see <<cp-subsystem, CP Subsystem>>.

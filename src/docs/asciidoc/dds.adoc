...
distributed implementations of many common Data Structures. For each of the client languages, Hazelcast will mimic
as closely as possible the natural interface of the structure. So, for example in Java the Map will follow
`java.util.Map` semantics. In the descriptions below we mention each structures Java equivalent interface.  All of
these structures are usable from Java, .Net, C++, Node.js, Python, Go & Scala.

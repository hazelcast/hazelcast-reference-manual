...
In order to support bigger and more affordable store for data structures like IMap, ICache and NearCache, Hazelcast provides integration with
persistent memory technologies like Intel Optane DC. To benefit from the technology there is no need to make any application code changes.
Only a few configuration changes are required.

A `native-memory` configuration block is extended with an optional element `persistent-memory-directory` which defines  where Optane memory
is mounted. The option enables usage of persistent memory on the cluster member so that all HD data structures will use the specified mounting
directory to store data. If the `persistent-memory-directory` is not configured, standard RAM is used.

The below snippet demonstrates how to configure the persistent memory as HD memory in Hazelcast.
...
The following code shows Java API to set persistent memory directory.

...
The `hazelcast-member` tool is a command line utility that can run one or more Hazelcast member(s) on the local machine. Below sections provide its installation instructions for various platforms and usage details.

==== Installing the Tool

The `hazelcast-member` tool is packaged in `yum` for Red Hat and CentOT, `apt` for Debian and Ubuntu, and `brew` for Mac OS X. See the below sections for installation details.
...
Run the following command to start a Hazelcast member instance.
...
The process naming (`confident_tharp` in the above example) is for your convenience. As you start other members, you will see different names for each of them. You can start as many members as you want using the above `hazelcast-member start` command.

You can use the arguments described in the below table for the `start` command:
...
```
  ...
  -c or --config <file>
        Use <file> for Hazelcast configuration.
        If <file> is '-', read the standard input.

  -cn or --cluster-name <name>
        Use the specified cluster <name> (default: 'dev').

  -cp or --cluster-password <password>
        Use the specified cluster <password> (default: 'password').

  -fg or --foreground
        Run in the foreground.

  -i or --interface <interface>
        Bind to the specified <interface> (default: bind to all interfaces).

  -j or --jar <path>
        Add <path> to Hazelcast class path.

  -p or --port <port>
        Bind to the specified <port> (default: 5701).

  -v or --verbose
        Show extra info about running environment.

  -J or --JAVA_OPTS <options>
        Specify additional Java <options>.

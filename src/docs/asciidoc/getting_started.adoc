...
==== Installing Hazelcast on macOS with Homebrew

Assuming you have the link:https://brew.sh/[Homebrew^] package manager, run the following commands:
...
==== Installing Hazelcast with Debian Package

Run the commands below to install Hazelcast using Debian package. It downloads the package from our own APT repository:
...
==== Installing Hazelcast with RPM

Run the following commands to install Hazelcast using link:https://rpm.org/[RPM^]:

[source,bash,subs="attributes+"]
----
$ wget https://bintray.com/hazelcast/rpm/rpm -O bintray-hazelcast-rpm.repo
$ mv bintray-hazelcast-rpm.repo /etc/yum.repos.d/
$ yum install hazelcast
----

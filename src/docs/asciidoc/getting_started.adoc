...
The MBean `HazelcastInstance.LicenceInfo` holds all the relative license info and can be accessed through Hazelcast's JMX port (if enabled).

- `maxNodeCountAllowed`: Maximum nodes allowed to form a cluster under the current licence
- `expiryDate`: The expiry date of the current licence
- `typeCode`: The type code of the current licence
- `type`: The type of the current licence
- `ownerEmail`: The email of the owner on the current licence
- `companyName`: The name of the company on the current licence

*Appendix:*

License types and their codes.
```
...
Through the REST API (if enabled) on the following resource `/license` on can access the info by issuing a `GET` request.
...
licenceInfo{"expiryDate":1560380399161,"maxNodeCount":10,"type":-1,"companyName":"ExampleCompany","ownerEmail":"ExampleOwner"}

```

===== Logs

Except from the above approaches to access the license info, when the license expiration is approaching, we will start logging a license info banner on the logfiles.

In the last 2 months prior to the expiration, the bannger will be logged daily, as a reminder to renew your license to avoid any interruptions. Once the expiration is due to a month, the frequency will again increase, and the banner will be logged hourly. Last, when the expiration is due in a week, the banner will be printed every 30 minutes.

IMPORTANT: Similar alerts will also be present on the Management Center.

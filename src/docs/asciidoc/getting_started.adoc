...
This chapter offers a quick start section thru which
you start Hazelcast members, form a cluster, connect it with
a client application and monitor the cluster using Hazelcast Management Center.
Then it provides pre- and post-installation
details and deployment options.

=== Quick Start

This section covers a basic Hazelcast installation,
starting the members and Management Center, and creating an application in your
preferred language.
...
to start a Hazelcast member while you are in the extracted directory:
...
[source,csharp,indent=0,subs="verbatim,attributes",role="secondary"]
.C#
----
...
client libraries of Hazelcast IMDG.
...
NOTE: The above steps let you to use both IMDG members and Java clients. A separate
Java client module does not exist. See <<removal-of-hazelcast-client-module, here>>
for the details.
...
The above repositories include both IMDG members and Java clients. A separate
Java client module and dependency do not exist. See <<removal-of-hazelcast-client-module, here>>
for the details.

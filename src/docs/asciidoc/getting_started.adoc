...
To update the license of a running cluster, you can issue a `POST` request through the REST API (if enabled; see the <<using-the-rest-endpoint-groups, Using the REST Endpoint Groups>> section) on the `/license` (note that the request parameters must be properly URL-encoded as described in <<rest-client, REST Client>>):

```
curl --data "${GROUPNAME}&${PASSWORD}&${LICENSE}" http://localhost:5001/hazelcast/rest/license
```

This command will update the license on all running Hazelcast members of the cluster. If successful, the response will look as follows:
...
It is only possible to update to a license that expires at the same time or after the current license. The new license must allow the exact same list of features and the same number of nodes.

If, for any reason, updating the license fails on some member (member does not respond, license not compatible, etc.), the whole operation will fail, leaving the cluster in a potentially inconsistent state (some members have been switched to the new license while some have not). It is up to the user to resolve this situation manually.

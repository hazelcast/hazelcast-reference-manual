...
If you want to deploy your own code to the server, you can use `/user-code` folder.

Let's say you use Hazelcast Node.js Client and you want to use an `EntryProcessor`. The processor should be `IdentifiedDataSerializable` or `Portable` in the Node.js client. You need to implement the Java equivalents of the processor and its factory on the server side and add them to the `CLASSPATH`. To make it easily, you can put your Java files to `/user-code` folder.

**IdentifiedEntryProcessor.java**
```
...
You need to configure `hazelcast.xml` file in the `/bin` folder. In the serialization tag, add your `IdentifiedFactory` like the following:
...
Then, you can start your Hazelcast member by using the start scripts in the `/bin` folder. The start scripts will automatically add your compiled classes to the `CLASSPATH`.

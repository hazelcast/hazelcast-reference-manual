...
To update the license of a running cluster, you can issue a `POST` request through the REST API (if enabled; see the <<using-the-rest-endpoint-groups, Using the REST Endpoint Groups section>>) on the `/license`:

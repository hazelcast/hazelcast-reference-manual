...
See the <<discovering-members-auto-detection, Discovering Members by Auto Detection>> for more details.
...
discovery plugin and apply it to into your Hazelcast configuration. For example, assuming you have `hazelcast-all`
...
* IAM role with the `ec2:DescribeInstances` permission attached to EC2 Instance
...
| Kubernetes requires applying the following

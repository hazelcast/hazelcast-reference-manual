...
This discovery mechanism provides a service based discovery strategy by using Apache Curator to communicate with your Zookeeper server. You can use this plugin with <<discovery-spi, Discovery SPI>> enabled applications. This is provided as a Hazelcast plugin. Please see its https://github.com/hazelcast/hazelcast-zookeeper/blob/master/README.md[documentation] for information on configuring and using it.
...
Please also see the https://github.com/hazelcast/hazelcast-code-samples/tree/master/hazelcast-integration/openshift[Hazelcast for OpenShift] guide, which presents how to set up the local OpenShift environment, start a Hazelcast cluster, configure the Management Center and finally run a sample client application.
...
control the classes to be served to other members and create blacklists or whitelists of classes and packages. When you enable this feature, you don't need to deploy your classes to all cluster members.
...
. Finally, dynamic class loader checks the remote class repository. If a member in this repository returns the class, it means your class is found and to be used. You can also put this class into your local class cache as mentioned in the previous step.
...
* `enabled`: Specifies whether dynamic class loading is enabled or not. Its default value is "false" and it is a mandatory attribute.
...
* `<blacklist-prefixes>`: Comma separated name prefixes of classes/packages to be prevented from dynamic class loading. For example, if you set it as "com.foo", remote loading of all classes from the "com.foo" package is prevented, including the classes from all its sub-packages. If you set it as "com.foo.Class", then the "Class" and all classes having the "Class" as prefix in the "com.foo" package are blacklisted. There are some built-in prefixes which are blacklisted by default. These are as follows:
** `javax.`
** `java.`
** `sun.`
** `com.hazelcast.`
...
* `<provider-filter>`: Filter to constraint members to be used for a class loading request when a class is not available locally. The value is in the format "HAS_ATTRIBUTE:foo". When it is set as "HAS_ATTRIBUTE:foo", the class loading request is only sent to the members which have "foo" as a <<defining-member-attributes, member attribute>>. Setting this to null allows to load classes from all members. Please see an example in the below section.

[[example-for-filtering-members]]
==== Example for Filtering Members

As described above, the configuration element `provider-filter` is used to constrain a member to load classes only from a subset of all cluster members. The value of the `provider-filter` must be set as a member attribute in the desired members from which the classes are to be loaded. Please see the following example usage provided as programmatic configurations.
...
The remaining properties, which are `blacklist-prefixes`, `whitelist-prefixes` and `provider-filter` configured at the member side, effect the client user code deployment's behavior too. For example, assuming that you provide `com.foo` as a blacklist prefix at the member side, the member discards the classes with the prefix `com.foo` loaded by the client.
...
NOTE: When using the ZONE_AWARE partition grouping, a Hazelcast cluster spanning multiple AZs should have an equal number of members in each AZ. Otherwise, it results in uneven partition distribution among the members.
...
NOTE: Multicast mechanism is not recommended for production since UDP is often blocked in production environments and other join mechanisms are more definite.
...
NOTE: This SPI is not intended to provide addresses of other cluster members with which the Hazelcast instance forms a cluster. To do that, refer to the other network configuration sections above.
...
The `hazelcast.heartbeat.interval.seconds` and `hazelcast.max.no.heartbeat.seconds` properties still can be used as period of heartbeat messages and deadline of heartbeat messages. Since _Phi Accrual Failure Detector_ is adaptive to network conditions, a much lower `hazelcast.max.no.heartbeat.seconds` can be defined than _Deadline Failure Detector_'s timeout.
...
| false
| false
| Legacy ping mode. This works hand-to-hand with the OSI Layer 7 failure detector (see. phi or deadline in sections above). Ping in this mode only kicks in after a period when there are no heartbeats received, in which case the remote Hazelcast member is pinged up to a configurable count of attempts. If all those attempts fail, the member gets suspected. You can configure this attempt count using the <<max-attempts, hazelcast.icmp.max.attempts>> system property.
...
Up to and including Hazelcast 3.11, Hazelcast members use a single server socket for all kinds of connections: cluster members, Hazelcast binary client protocol clients and HTTP protocol clients connect to a single server socket that handles all protocols.

Starting with Hazelcast 3.12, it is possible to configure Hazelcast members with separate server sockets using different network configuration for different protocols. This configuration scheme allows for more flexibility when deploying Hazelcast as described in the following examples:

 * for security, it is possible to bind the member protocol server socket on a protected internal network interface, while client connections can be established on another network interface accessible by Hazelcast clients
 * different kinds of network connections can be established with different socket options, for example varying send/receive window size to optimize network usage, configure TLS for connections over WAN while member-to-member connections may remain unencrypted etc.

In the following example we introduce the advanced network configuration for a member to listen for member-to-member connections on default port 5701 while listening for client connections on port 9090 like this:
...
Running this example will print something similar to the following output indicating the server listens for the specified protocols on the respective configured ports:
```
{EndpointQualifier{type='CLIENT'}=[10.212.134.156]:9090, EndpointQualifier{type='MEMBER'}=[10.212.134.156]:5701}
```

This is the equivalent declarative configuration:
...
Advanced network configuration and single-socket network config are mutually exclusive: either an enabled `AdvancedNetworkConfig` or the `NetworkConfig` object is used to configure a member's networking, including the joiner, discovery, failure detectors etc described above. It is an error to define both elements in declarative config; in programmatic config, an enabled `AdvancedNetworkConfig` will take precedence over `NetworkConfig`. By default `AdvancedNetworkConfig` is disabled, therefore the unisocket member configuration under `NetworkConfig` is used.

When using advanced network configuration, the following configuration is defined member-wide:

 * Joiner & cluster discovery (multicast, TCP/IP, AWS, Eureka etc)
 * `MemberAddressProvider` configuration
 * Failure detector configuration

In addition to the above, advanced network configuration allows the configuration of multiple endpoints: each endpoint configuration applies for a specific protocol (e.g. `MEMBER`, `CLIENT`, ...). An additional optional identifier can be configured, to separate configuration of multiple `WAN` protocol endpoints.

The supported protocols are:

 * `MEMBER`: a member server socket is required for Hazelcast to operate. The default advanced network configuration defined a member endpoint configuration listening on port 5701 (same as the unisocket Hazelcast member configuration).
 * `CLIENT`: a single server socket handling Hazelcast client binary protocol can be optionally configured. If no such endpoint is configured, then clients will not be able to connect to the Hazelcast member.
 * `REST`: a REST server socket is optional.
 * `MEMCACHE`: when accessing Hazelcast over memcache text protocol, an endpoint listening to `MEMCACHE` protocol must be defined.
 * `WAN`: multiple WAN endpoint configurations can be defined to determine network settings of outgoing connections (from members of a source cluster to target WAN cluster members) or to establish server sockets on which a target WAN member can listen for incoming connections from the source cluster.

==== Server socket endpoint configuration

The server socket endpoint configuration is common for all protocols. The elements comprising a server socket endpoint configuration are identical to their unisocket network configuration counterparts. Here are all the common server socket endpoint configuration elements for declarative configuration:
...
When using declarative configuration, specific element names introduce the server socket endpoint configuration for each protocol:
...
With programmatic config, corresponding methods set the respective server socket endpoint config:
...
In addition to the common server socket configuration described above, REST endpoint configuration includes certain additional attributes specific to configuration of which groups of REST functionality are enabled.
...
Similarly, these additional elements are available in declarative configuration under `rest-server-socket-endpoint-config` element:
...
Multiple WAN endpoint configurations can be defined to configure outgoing connections and server sockets, depending on the role of the member in WAN replication. In the following sections configuration examples are provided for both active and passive side of WAN replication.

===== Configuring the WAN active side

Members on the active cluster initiate connections to target cluster members, so there is no need to create a server socket. A plain `EndpointConfig` is created that supplies configuration for client side of the connections that active members will create:
...
The same configuration as declarative:
...
The endpoint configuration includes all

===== Configuring the WAN passive side

On the passive cluster, a server socket is configured on members to listen for incoming WAN connections, matching the network configuration (e.g. SSL configuration etc) configured on the active side of WAN replication.
...
Q: Can I multiplex protocols on a single advanced network endpoint? For example, can I use a single server socket to listen for `MEMBER` and `CLIENT` protocols?

A: No, each endpoint configuration that defines a server socket must bind to a different socket address.

Q: Can I mix unisocket and advanced network members in the same cluster?

A: No, the results will be undefined.

Q: Can I configure multiple server socket endpoints for the same protocol?

A: You can only configure multiple server socket endpoints for `WAN` protocol. For other protocols (`MEMBER`, `CLIENT`, `REST`, `MEMCACHE`), a single server socket can be configured.

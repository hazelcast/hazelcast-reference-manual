...
- **User Search Filter:** LDAP search filter expression to search for users. `uid={0}` searches for a username match on the `uid` attribute.
- **Group Search Filter:** LDAP search filter expression to search for groups. `uniquemember={0}` searches for a group match on the `uid` attribute.

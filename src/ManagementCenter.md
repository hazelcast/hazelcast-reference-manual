...
The window titled **Partitions** shows which partitions are assigned to the selected member. **Runtime** is a dynamically updated window tab showing the processor number, the start and up times, and the maximum, total and free memory sizes of the selected member. These values are getting collected from the default MXBeans which JVM provides. Descriptions from javadocs and some explanations are below:

- **Number of Processors**: Number of processors available to the Java virtual machine.

- **Start Time**: Start time of the Java virtual machine in milliseconds.

- **Up Time**: Uptime of the Java virtual machine in milliseconds

- **Maximum Memory**: Maximum amount of memory that the Java virtual machine will attempt to use.

- **Free Memory**: Amount of free memory in the Java Virtual Machine.
...
- **Total Loaded Classes**: Total number of classes that have been loaded since the Java virtual machine has started execution.

- **Current Loaded Classes**: Number of classes that are currently loaded in the Java virtual machine.

- **Total Unloaded Classes**: Total number of classes unloaded since the Java virtual machine has started execution.

- **Total Thread Count**: Total number of threads created and also started since the Java virtual machine started.

- **Active Thread Count**: Current number of live threads including both daemon and non-daemon threads.

- **Peak Thread Count**: Peak live thread count since the Java virtual machine started or peak was reset.
...
- **OS: Free Swap Space**: Amount of free swap space in bytes. (used when the amount of physical memory (RAM) is full. If the system needs more memory resources and the RAM is full, inactive pages in memory are moved to the swap space.)

- **OS: Total Swap Space**: Total amount of swap space in bytes.

- **OS: Maximum File Descriptor Count**: Maximum number of file descriptors. (File descriptor is an integer number that uniquely represents an opened file in operating system.)

- **OS: Open File Descriptor Count**: Number of open file descriptors.

- **OS: Process CPU Time**: CPU time used by the process on which the Java virtual machine is running in nanoseconds.

- **OS: Process CPU Load**: Recent cpu usage for the Java Virtual Machine process. This value is a double in the [0.0,1.0] interval. A value of 0.0 means that none of the CPUs were running threads from the JVM process during the recent period of time observed, while a value of 1.0 means that all CPUs were actively running threads from the JVM 100% of the time during the recent period being observed. Threads from the JVM include the application threads as well as the JVM internal threads.
...
Next to this, the **Properties** tab shows the system properties. The **Member Configuration** window shows the connected Hazelcast cluster's XML configuration.

...
<br>
![Note](images/NoteSmall.jpg) ***NOTE:*** *Note that you may want to use [Hazelcast Jet](https://jet.hazelcast.org/) if you want to process batch or streaming data. Hazelcast Jet is a distributed computing platform for in-memory batch and stream processing. Please see the [Fast Batch Processing and Stream Processing](???link to fast batch processing and stream processing on .com ???) use cases for Hazelcast Jet.*
